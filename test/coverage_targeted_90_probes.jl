...
@testset "Coverage Targeted >=90 Probes" begin
    ...
    @testset "from_env parser and restore branches" begin
        ...
        # simulation_engine_config_from_env's general/introspection contract defaults
        # to solver_strict=false (a malformed individual solver knob falls back to its
        # default rather than crashing whole-config construction; see from_env.jl).
        # Malformed-value rejection is still exercised here, via the same solver_strict=true
        # path _active_solver_config uses for the solver hot path.
...
@testset "Density Freeze Per Step Branch Probes" begin
    ...
    p_freeze = ODEParams{1}(args=args_freeze)
    ...
    p_freeze_unset = ODEParams{1}(args=args_freeze)
    withenv("SPACEAGORA_DENSITY_FREEZE_PER_STEP" => "1") do
