...
# GRAM access
If part of the Space-FALCON Lab, access through the lab's Drive in SpaceAGORA/GRAM. Use the `GRAMSuite.jl/GRAM Suite 2.0` folder at the repository root and build the shared library with the scripts in `GRAMSuite.jl/GRAM Suite 2.0/simulation/GRAM/`. Treat `GRAMSuite.jl/GRAM Suite 2.0/Build/` as generated, host-specific output and rebuild it natively on each machine.
...
* Change the GRAM root/data/SPICE paths to match your system. They should point to your local `GRAMSuite.jl/GRAM Suite 2.0` installation.
