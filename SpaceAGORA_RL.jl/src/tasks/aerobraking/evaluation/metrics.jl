...
function aggregate_metrics(summaries::Vector{EpisodeSummary}; policy_name::AbstractString="")
    ...
    return (
        ...
        mean_apoapsis_correction_delta_v_mps = mean(getfield.(summaries, :apoapsis_correction_delta_v_mps)),
        mean_periapsis_raise_delta_v_mps = mean(getfield.(summaries, :periapsis_raise_delta_v_mps)),
        mean_total_mission_delta_v_mps = mean(getfield.(summaries, :total_mission_delta_v_mps)),
