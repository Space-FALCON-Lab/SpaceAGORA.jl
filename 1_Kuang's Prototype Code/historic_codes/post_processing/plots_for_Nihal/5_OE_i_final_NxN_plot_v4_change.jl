...
for (row_idx, h) in enumerate(target_altitudes_km)
    ...
    all_ifinals = Vector{Vector{Float64}}()
    ...
    for i_deg in target_inclinations_deg
        ...
        if res === nothing
            ...
            push!(all_ifinals, i_final_deg[ord])
        end
    end

    all_vals_flat = vcat(all_ifinals...)
    ...
    _imin = minimum(all_vals_flat); _imax = maximum(all_vals_flat)
    _ictr  = (_imin + _imax) / 2
    _ihalf = max((_imax - _imin) / 2 * 1.1, abs(_ictr) * 0.5, 1e-9)
    ytk_vals, ytk_labs = _is_all_zero ? nice_ticks(-0.0020, 0.0020; n=5) :
    ...
    for (k, (xs, yf)) in enumerate(zip(all_xs, all_ifinals))
        isempty(xs) && continue
        plot!(sp, xs, yf;
