...
### ORACLE Scenario Bundles

The workspace `ORACLE/run_case2_laser_links.jl` runner packages native output as
`trajectory.feather`, `geometry_encounters.feather`, and `laser_on.feather` under
`output/feather/<scenario>/`. Its manifest references all three files. Trajectory
output is every 10 seconds plus the final endpoint; `timeseries_points` controls
buffering rather than the output interval. Geometry uses continuous range roots;
laser intervals follow range gates and the accepted-step scheduler.

`--smoke` runs 60 seconds and writes to `output/smoke/feather/<scenario>/`.
Unless `--feather-only` is set, the workspace's shared reader exports all three
tables and encounter summaries into SpaceAGORA's own
`output/CSV/<scenario>/`, or `output/smoke/CSV/<scenario>/` for smoke runs.
Direct table exports are in `sim_output/`; derived encounter reports and
`summary.csv` are in `analysis/`. The workspace-root `run_comparison.jl` can run
the prototype and SpaceAGORA from one case definition and writes a root-level
`comparison_summary.md` after reading both sets of saved analysis CSVs.
The old single-case/paper-grid mirroring is no longer used by this runner.
This bundle layout is ORACLE-specific; other `run_simulation` clients retain the
standard filenames described above.

The detailed schema and timing limitations are documented in the workspace's
`1_Kuang's Prototype Code/FEATHER_ENCOUNTERS.md`.
