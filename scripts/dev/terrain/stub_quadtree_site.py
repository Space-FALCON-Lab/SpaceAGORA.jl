...
`fetch_moon_site.py` writes five nested square images centered on the landing
site (`imagery/imagery.json`). The renderer in `viewer/src/terrain.js` instead
...
before the real tile pyramid lands:
  ...
  * the global surface texture (`data/textures/moon_lroc_wac_8k.jpg`) feeds the
...
class Source:
    ...
    def crop(self, lat_min, lat_max, lon_min, lon_max, px):
        ...
        tile = self.image.resize((px, px), Image.BICUBIC, box=box)
...
def main(argv=None):
    ...
    ap.add_argument("--global-texture", default="data/textures/moon_lroc_wac_8k.jpg")
