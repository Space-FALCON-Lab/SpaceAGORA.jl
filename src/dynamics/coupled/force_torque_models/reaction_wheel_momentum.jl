...
struct WheelSpeedSpline
    ...
    function WheelSpeedSpline(t_s::Vector{Float64}, y::Vector{Float64}, d2::Vector{Float64})
        n = _validate_wheel_samples(t_s, y)
        length(d2) == n || throw(ArgumentError("WheelSpeedSpline: d2 must match the knots."))
        all(isfinite, d2) || throw(ArgumentError("WheelSpeedSpline: d2 must be finite."))
        return new(copy(t_s), copy(y), copy(d2))
    end
end

function _validate_wheel_samples(x::Vector{Float64}, v::Vector{Float64})
    n = length(x)
...
WheelSpeedSpline(t_s::AbstractVector{<:Real}, y::AbstractVector{<:Real}) =
    wheel_speed_spline(t_s, y)
...
function of time. Use it for recorded or prescribed wheel histories, with
`orientation_sim=true`. Do not also represent those same physical wheels in the
spacecraft's commanded `rw_assembly`; that would count their momentum twice.
The model requests no atmosphere or ephemerides.

The engine's indexed hook applies the model only at `spacecraft_index` (the run
array index, not a spacecraft ID). Direct `wrench(model, x, env, t)` evaluates
this model for `x` without index selection or diagnostic mutation.
...
function ReactionWheelMomentumModel(
    ...
    all(isfinite, spin_axes) || throw(ArgumentError("ReactionWheelMomentumModel: spin_axes must be finite."))
    offset = Float64(time_offset_s)
    isfinite(offset) || throw(ArgumentError("ReactionWheelMomentumModel: time_offset_s must be finite."))
    length(state.momentum_body) >= spacecraft_index &&
        length(state.torque_body) >= spacecraft_index || throw(ArgumentError(
        "ReactionWheelMomentumModel: both diagnostic vectors must contain spacecraft_index."))
    ...
    all(axis -> all(isfinite, axis), axes) || throw(ArgumentError(
        "ReactionWheelMomentumModel: scaled momentum axes must be finite."))
    splines = [wheel_speed_spline(t_s, @view speeds_rad_s[:, k]) for k in 1:n_wheels]
    return ReactionWheelMomentumModel(spacecraft_index, axes, splines, offset, state)
end

"""
    wheel_speeds_rad_s(model, t) -> Vector{Float64}

Interpolated signed speeds for every wheel, in input column order, at run time
`t`. This diagnostic allocates its result; the momentum/torque path does not.
"""
function wheel_speeds_rad_s(model::ReactionWheelMomentumModel, t::Float64)::Vector{Float64}
    tt = t + model.time_offset_s
    return [wheel_spline_value(spline, tt) for spline in model.speeds]
end
...
# The context-free public hook is pure. A StateSample carries a spacecraft
# object but no run index, so target selection belongs to wrench_caching!.
function wrench(
    ...
    _, torque = _reaction_wheel_values(model, x, t)
...
function wrench_caching!(
    ...
    h, torque = _reaction_wheel_values(model, x, t)
    # Constructor checks both lengths. Keep these diagnostic writes checked
    # because the caller can still resize a supplied state's vectors later.
    model.state.momentum_body[sat_idx] = h
    model.state.torque_body[sat_idx] = torque
    return (zero3, torque)
end

@inline function _reaction_wheel_values(model::ReactionWheelMomentumModel, x::StateSample, t::Float64)
    zero3 = SVector{3, Float64}(0.0, 0.0, 0.0)
    omega = x.ω_body
    omega === nothing && return (zero3, zero3)
    h = wheel_momentum_body(model, t)
    hdot = wheel_momentum_rate_body(model, t)
    return h, wheel_reaction_torque(h, hdot, omega)
end
