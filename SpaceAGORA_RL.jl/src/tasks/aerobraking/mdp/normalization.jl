...
function normalize_observation(values::AbstractVector{<:Real}, bounds::NormalizationBounds)
    length(values) == length(bounds.lower) ||
        throw(DimensionMismatch("observation length does not match bounds"))
    return [normalize_value(values[i], bounds.lower[i], bounds.upper[i]) for i in eachindex(values)]
end

normalize_observation(obs::PaperObservation, bounds::NormalizationBounds) =
    normalize_observation(raw_observation_vector(obs), bounds)
