...
args = Dict(# Misc Simulation
            ...
            :directory_results => "/workspaces/SpaceAGORA.jl_deprecated/journal_output/Venus_targ_pert_mc",                # Directory where to save the results
            ...
            :topography_harmonics_file => "/workspaces/SpaceAGORA.jl_deprecated/Topography_harmonics_data/MGN-V-RDRS-5-TOPO-L2.csv", # File with the topography harmonics coefficients
            ...
            # Perturbations
            :n_bodies => ["Sun"], # ["Sun"],                                  # Add names of bodies you want to simulate the gravity of to a list. Keep list empty if not required to simulate extra body gravity.
            :srp => 0,                                             # Solar Radiation Pressure True=1, False=0
            :gravity_harmonics => 0,                               # Gravity Harmonics True=1, False=0
            :gravity_harmonics_file => "/workspaces/SpaceAGORA.jl_deprecated/Gravity_harmonics_data/MGNP180U.csv", # File with the gravity harmonics coefficients
            ...
            :max_heat_load => 15.0,                                 # Max heat load the heat load control will not be overcomed
            ...
            :ra_initial_a => 72649e3, # 66197e3 + 6.0518e6, # 28523.95e3,                # Initial Apoapsis Radius for for-loop in m
            :ra_initial_b => 1e8,                               # Final Apoapsis Radius for for-loop in m
            :ra_step => 5e21,                                       # Step Apoapsis Radius for for-loop in m
            :hp_initial_a => 136e3, # 186600.0,#176590.0,#188140.0                                 # Initial Periapsis Altitude for for-loop in m
            ...
            :ra_fin_orbit => 72480e3, # 68550.8e3, # 60605e3 + 6.0518e6,                    # Target final apoapsis for the orbit, m
...
#     for ra in [10000e3, 15000e3, 25000e3, 40000e3] # [10000e3, 15000e3, 25000e3, 40000e3]
#         ra_initial_a = ra
#         hp_initial_a = 120e3
#         inc          = args[:inclination]
#         RAAN         = args[:Ω]
