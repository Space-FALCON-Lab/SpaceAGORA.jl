...
plt1 = scatter(title="Net Satellite Angular Momentum ($dir_tag)", xlabel="t (s)",
               ylabel="Total Angular Momentum (kg·m²/s)", formatter=:scientific, legend=:outertopright)
plt2 = scatter(title="Angular Momentum Variation ($dir_tag)", xlabel="t (s)",
               ylabel="ΔH = H(t) - H(0) (kg·m²/s)", formatter=:scientific, legend=:outertopright)
...
for csv_path in csv_files
    ...
    ΔH   = Hmag .- Hmag[1]
    N = p[:N]
    scatter!(plt1, sol.t, Hmag, label="N=$N", ms=2, markerstrokewidth=0)
    scatter!(plt2, sol.t, ΔH,   label="N=$N", ms=2, markerstrokewidth=0)
end
...
savefig(plt2, joinpath(output_dir, "angular_momentum_variation_$(dir_tag).png"))
