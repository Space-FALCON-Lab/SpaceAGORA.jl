...
args = Dict(# Misc Simulation
            ...
            :dry_mass => 1100.0,                                     # Initial dry mass of body in kg
            :prop_mass => 0.1,                                     # Initial propellant mass of body in kg
            ...
            :base_radius => 3.0,                                 # Base radius of the blunted cone in m
            :nose_radius => 0.525,                                 # Nose radius of the blunted cone in m
            ...
            :v_initial_a => 7.9466e3,                                 # Initial Velocity (m/s) for for-loop if initial conditions are in v and gamma
            ...
            :γ_initial_a => 2.2933,                                    # Initial Gamma (deg) for for-loop if initial conditions are in v and gamma
            :γ_initial_b => 7.0,                                    # Final Gamma (deg) for for-loop if initial conditions are in v and gamma
            :γ_step => 15.5,                                         # Step Gamma (deg) for for-loop if initial conditions are in v and gamma
            
            :orientation_type => 1,                                   # Initial orientation type: 0=OE, 1=Lat/Lon
            :inclination => 89.876,                                   # Inclination Orbit, deg
            :ω => 75.505,                                              # AOP, deg
            :Ω => 104.115,                                              # RAAN, deg
            :latitude => 33.8818,                                     # Initial Geodetic Latitude (deg)
            :longitude => 214.2874,                                        # Initial Longitude (deg)
            :azimuth => 189.6399,                                        # Initial Azimuth (deg)
            :EI => 125.0,                                           # Entry Interface, km
            :AE => 125.0,                                           # Atmospheric Exit, km
            :year => 2022,                                          # Mission year
            :month => 11,                                           # Mission month
            :day => 10,                                             # Mission day
            :hours => 11,                                           # Mission hour
            :minutes => 15,                                         # Mission minute
            :secs => 0.0,                                          # Mission second
            
            # Final Conditions
            :final_apoapsis => 1, # 4905.974818462152e3                  # Final apoapsis radius if aerobraking campaign
            :final_altitude => 18e3,                                     # Final altitude if entry, m
            # Do not change
            ...
            :ra_fin_orbit => 0,                               # Target final apoapsis for the orbit, m
            :maneuver_plan => nothing,                    # Firing plan function
