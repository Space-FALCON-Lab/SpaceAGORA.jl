...
println("Data timestep: $(abs(torque_rod_test_data[!, "TimeData"][1] - torque_rod_test_data[!, "TimeData"][2]))")
...
time_range = range(torque_rod_test_data[!, "TimeData"][1], torque_rod_test_data[!, "TimeData"][end], length(torque_rod_test_data[!, "TimeData"]))
rw_1_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "RWMotorTorque[0]"])
rw_2_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "RWMotorTorque[1]"])
rw_3_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "RWMotorTorque[2]"])
rw_4_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "RWMotorTorque[3]"])
rw_torque_itp = (t) -> SVector{4, Float64}([rw_1_itp(t), rw_2_itp(t), rw_3_itp(t), rw_4_itp(t)])
println(rw_torque_itp(0.1))
...
tr_x_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "MTBDipoleCmds[0]"])
tr_y_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "MTBDipoleCmds[1]"])
tr_z_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "MTBDipoleCmds[2]"])
tr_trans_itp = cubic_spline_interpolation(time_range, torque_rod_test_data[!, "MTBDipoleCmds[3]"])
...
main_bus = config.Link(root=true, 
                        ...
                        max_torque=10.0, # max torque from RW datasheet, Nm
                        max_h=0.015, # max angular momentum from RW datasheet, Nms
                        J_rw=J_rw,
...
TRT = config.Magnet(axis=MVector{3, Float64}(0.70710678, 0.70710678, 0.0), magnitude=0.0)
...
args = Dict(# Misc Simulation
            ...
            :ω => 0.0,                               # AOP, deg, starting from 0s
            ...
            :a_tol_orbit => 1e-12,                                    # Absolute tolerance for orbit integration (outside atmosphere, i.e., step 1 and step 3)
            :r_tol_orbit => 1e-10,                                    # Relative tolerance for orbit integration (outside atmosphere, i.e., step 1 and step 3)
