...
| `PS_NO_SPICE` | Build the workload's Earth from the built-in constants and `SimpleEphemeridesModel` instead of the SPICE-backed constructor. Set automatically when the SPICE bundle is absent, so a baseline no-GRAM checkout runs S1 unchanged; set it explicitly on a machine that does have kernels to reproduce that path. Within 2% of the SPICE path at N=256 (serial 1.869 vs 1.903 s, parallel 0.806 vs 0.791 s) and above, where per-satellite work dominates. **Not** at the small rungs: SPICE does one locked `pxform` per distinct RHS time, a fixed per-solve cost, so it runs ~11% slower at N=64 and ~1.23x slower at N=4. The `spice` column in the CSV records which path produced each row. A `PS_DENSITY=gram*` mode still requires the real bundle and errors without it | auto |
