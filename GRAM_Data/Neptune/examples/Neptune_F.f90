...
PROGRAM Neptune_F
  ...
  call tryGetSpicePath_N(spicePath, 500);
  call initialize_N(spicePath)
  ...
  neptune = createAtmosphere_N()
  neptune2 = createAtmosphere_N()
  ...
  bufferSize = 100
  stringLength = getVersionString_N(neptune, version, bufferSize)
  write(*,*) version(1:stringLength)
  write(*,*)

  ! Set the minMaxFactor for the models
  call setMinMaxFactor_N(neptune, 0.5d0, 1)
  call setMinMaxFactor_N(neptune2,  -0.5d0, 1)

  ! Set the initial seed, the relative step size, and the perturbation scale factors.
  call setSeed_N(neptune, 1001)
  call setMinRelativeStepSize_N(neptune, 0.5D0)
  call setPerturbationScales_N(neptune, 1.5D0, 1.5D0, 1.5D0)
  call setSeed_N(neptune2, 3333)
  call setMinRelativeStepSize_N(neptune2, 0.5D0)
  call setPerturbationScales_N(neptune2, 0.5D0, 0.5D0, 0.5D0)

  ! Set dinitrogen mole fraction for neptune2 only
  call setDinitrogenMoleFraction_N(neptune2, 0.002d0)
  ...
  time%frame = 1
  time%scale = 1
  call setStartTime_N(neptune, time)
  call setStartTime_N(neptune2, time)
  ...
  pos%latitude = -22.0
  call setPosition_N(neptune2, pos)

  ! Update the model.
  errorFlag = update_N(neptune)
  errorFlag = update_N(neptune2)

  ! The position and other output is returned in the Position_F structure.
  call getPosition_N(neptune, pos)
  call getPosition_N(neptune2, pos2)
  write(*,*) "                           Neptune #1    Neptune #2"
  ...
  call getEphemerisState_N(neptune, estate)
  call getEphemerisState_N(neptune2, estate2)
  ...
  call getDynamicsState_N(neptune, dstate)
  call getDynamicsState_N(neptune2, dstate2)
  ...
  call getDensityState_N(neptune, rstate)
  call getDensityState_N(neptune2, rstate2)
  ...
  ! Get and print gases.
  call getGasesState_N(neptune, gstate, dihydrogen, methane, helium, dinitrogen)
  call getGasesState_N(neptune2, gstate2, dihydrogen2, methane2, helium2, dinitrogen2)
  ...
  call getWindsState_N(neptune, wstate)
  call getWindsState_N(neptune2, wstate2)
  ...
  call deleteAtmosphere_N(neptune)
  call deleteAtmosphere_N(neptune2)
