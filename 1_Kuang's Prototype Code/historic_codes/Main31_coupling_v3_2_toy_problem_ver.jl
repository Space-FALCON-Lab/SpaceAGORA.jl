...
target_altitudes_km = 1000 #850 # [950.0, 1000.0, 1050.0, 1150.0] # [850.0, 950.0, 1000.0, 1050.0, 1150.0] # [800]
target_inclinations_deg = 0.0 #[0.0, 0.5, 1.0] #[0.0, 0.5, 1.0, 1.5]
target_nu_degs = 1.0 #[-1.5, -0.75, 0.75, 1.5]  # true anomaly offset(s) for the target satellite
...
for alt_km in target_altitudes_km
    for inc_deg in target_inclinations_deg
        ...
        case_csv_dir = normpath(joinpath(@__DIR__, "output", "CSV",
                            @sprintf("target_h%.0fkm_i%.1fdeg_nu%.2fdeg", alt_km, inc_deg, nu_deg)))
        mkpath(case_csv_dir)
        ...
        for helper_num in helper_counts
            ...
            target_orbit = (a_m=R_EARTH + alt_km * 1e3, e=0.0, i_deg=inc_deg, Ω_deg=0.0, ω_deg=0.0, ν_deg=nu_deg)
            ...
            println(@sprintf("\nCase: target_h=%.0f km, target_i=%.1f deg, target_nu=%.2f deg | helpers=%d", alt_km, inc_deg, nu_deg, helper_num))
            ...
            save_timeseries_csv(sol, p, helper_oe, target_orbit, csv_dir=case_csv_dir,
                                include_B=true, include_Pin=true, include_Range=true, include_J2=true, include_toy=true)
        end
        end  # nu_deg
    end
end