...
cache_path = joinpath(output_dir, "dv_TN_cache_$(csv_tag).csv")
...
if isfile(cache_path)
    ...
    dv_T = raw[:, 3]
    dv_N = raw[:, 4]
    println("Loaded Δv TN cache: $cache_path")
else
    # ── Compute from CSV ─────────────────────────────────────────────────────
    ...
    open(cache_path, "w") do io
        writedlm(io, ["t" "orbits" "dv_T" "dv_N"], ',')
        writedlm(io, hcat(t, oc, dv_T, dv_N), ',')
    end
    println("Saved Δv TN cache: $cache_path")
end

# ── Plot T and N components on a single figure ──────────────────────────────
...
# ── Save ─────────────────────────────────────────────────────────────────────
save_path = joinpath(output_dir, "dv_TN_toy_problem_$(csv_tag).png")
