...
Base.@kwdef struct OracleOptions
    ...
    animate::Bool                   = false
    helper_trails::Union{Bool,Vector{Int}} = true
...
# Main #
########
println("\nSTART SIMULATION (test16_options — copied from test10):")
...
paths = scenario_paths(joinpath(@__DIR__, opts.output_dir), opts, sol.t[end];
    source="prototype", schedule=:none)
...
if opts.animate
    ...
    animate_all_satellites_3d_smooth_helper_target(sol, p, helper_num;
        tail=1000, show_earth=true, Δt=0.1, output_file=video_path,
