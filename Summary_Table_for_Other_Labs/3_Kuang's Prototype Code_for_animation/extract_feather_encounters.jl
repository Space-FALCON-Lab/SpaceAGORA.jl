...
function extract_feather_encounters(feather_path;
    ...
    length(spacecraft_ids) >= 2 && first(spacecraft_ids) == 1 ||
        throw(ArgumentError("At least two spacecraft including target sc1 are required"))
    ...
    if active_helpers !== nothing
        all(helper -> ismissing(helper) || helper == 0 || helper in spacecraft_ids[2:end], active_helpers) ||
    ...
    for first_index in 1:length(spacecraft_ids)-1, second_index in first_index+1:length(spacecraft_ids)
        ...
        for sample in eachindex(times), metric_index in 1:2, extremum_index in 1:2
            value = motions[sample][metric_index+1]
            ...
            if ismissing(previous) || (extremum_index == 1 ? value < previous : value > previous)
                extrema[row, [:value, :sc_a, :sc_b, :time_s]] = [value, sc_a, sc_b, times[sample]]
            end
        end
    ...
    for metric in ("geometry_duration_s", laser_metric), extremum in ("min", "max")
        ...
        if isempty(valid_rows)
            ...
            push!(extrema, (metric, extremum, encounter[metric], encounter.sc_a, encounter.sc_b,
    ...
    if event_bundle
        ...
        CSV.write(joinpath(sim_output_dir, "geometry_encounters.csv"), geometry)
        CSV.write(joinpath(sim_output_dir, "laser_on.csv"), laser_intervals)
    end
