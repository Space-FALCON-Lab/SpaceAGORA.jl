...
const SWEEP_ROOT = joinpath(
    ...
    "outputs",
    "hyperparameter_sweeps",
    "a2c_rollout_refinement_20260817",
)
...
const VARIANTS = [
    (
        name="rollout_length_15",
        label="Changed (rollout length = 15)",
        parameter="rollout_length",
        nominal_value="16",
        changed_value="15",
        values=merge(BASELINE, (segment_length=15,)),
    ),
    (
        name="rollout_length_17",
        label="Changed (rollout length = 17)",
        parameter="rollout_length",
        nominal_value="16",
        changed_value="17",
        values=merge(BASELINE, (segment_length=17,)),
    ),
]
...
Usage:
  julia --project=SpaceAGORA_RL.jl \\
    SpaceAGORA_RL.jl/scripts/run_a2c_rollout_refinement_sweep.jl [--generate-only]

Reuses the completed rollout-length-16 run as the baseline. Generates fresh
rollout-length-15 and rollout-length-17 configurations and, unless
--generate-only is supplied, trains and checkpoint-validates each incomplete
arm. Re-running skips arms with completed best-checkpoint validation.
""")
end
...
function _write_config(variant)
    ...
    path = joinpath(CONFIG_DIR, "a2c_refinement_$(variant.name).toml")
    open(path, "w") do io
...
function _write_definition(config_paths, baseline_run)
    ...
    changed_records = [
        Dict(
            ...
            "analysis_directory" => "rollout_length_$(variant.changed_value)",
    ...
    definition = Dict(
        ...
        "design" => "rollout_length_refinement_from_16",
...
function main(args=ARGS)
    ...
    for (variant, config_path) in zip(VARIANTS, config_paths)
        ...
        println("starting rollout refinement variant ", variant.name)
