...
function make_prototype_recorder(initial_state, params)
    order = vcat(params[:helper_ids], params[:target_ids])
...
names and SI units. Helpers precede the single target, which is scN.
...
function save_timeseries_feather(sol, p;
    ...
    satellite_order = vcat(p[:helper_ids], p[:target_ids])
    ...
    active_helpers = map(sol.u) do state
        ...
        helper_index === nothing ? 0 : helper_index
    end
