...
{
  cout << "===========" << endl;
  cout << "The EarthCorrMonte Program" << endl;
  ...
  try {
    ...
    // Start with a CorrMonte object
    CorrMonte corrMonte;

    // The CorrMonte object requires an atmospheric state correlator.
    // Here we use and EarthCorrelator.
    EarthCorrelator correlator;
    correlator.setMean(inputParameters.corrMean);
    corrMonte.setCorrelator(correlator);
    ...
    trajProfile.setInputParameters(inputParameters);

    // The profile is ready, give it to the Monte Carlo object
    corrMonte.setProfile(trajProfile);
    ...
    if (inputParameters.useLegacyOutputs) {
      ...
      printer.setStyle(printer.EARTH_CORR_STYLE);
    }
