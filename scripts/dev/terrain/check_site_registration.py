...
places either side of a level change. Three diagnostic routes:
...
1. **Against the digital terrain model.** The imagery is resampled onto the DTM's own grid and
   ...
   This measures relative registration; it does not by itself establish which product is
   responsible for an offset.
...
All three routes are reported in meters north and east, in the sense `LAYER_REGISTRATION` uses: the offset
...
def load_grid(meta_path):
    ...
    if meta.get("reference_radius_m") != fms.MOON_RADIUS_M:
        raise ValueError("Registration DEM must declare the 1737400 m sphere.")
    if not fms.existing(pathlib.Path(meta_path).with_suffix("")):
        raise ValueError("Registration DEM files are incomplete.")
    data = np.fromfile(pathlib.Path(meta_path).with_suffix(".f32"), dtype="<f4").reshape(meta["rows"], meta["cols"])
    if not np.isfinite(data).all():
        raise ValueError("Registration DEM must contain finite heights.")
    return meta, data
...
def hillshade(heights, m_per_cell, sun_azimuth_deg, sun_elevation_deg):
    ...
    spacing = (m_per_cell,m_per_cell) if np.isscalar(m_per_cell) else tuple(m_per_cell)
    if len(spacing) != 2 or not all(math.isfinite(v) and v > 0 for v in spacing):
        raise ValueError("Hillshade requires positive north/east cell spacings.")
...
def phase_correlate(a, b):
    ...
    if a.ndim != 2 or a.shape != b.shape or min(a.shape) < 3 or not np.isfinite(a).all() or not np.isfinite(b).all() or a.std() == 0 or b.std() == 0:
        raise ValueError("Correlation needs equally shaped finite 2-D arrays with nonzero contrast.")
...
def against_dem(source, dem_meta, dem, lat, lon, box_m, azimuths, sun_elevation_deg):
    ...
    east_per_cell = dlon*fms.M_PER_DEG*math.cos(math.radians(lat))
    if min(m_per_cell,east_per_cell,box_m) <= 0 or not all(map(math.isfinite,(m_per_cell,east_per_cell,box_m))):
        raise ValueError("Registration requires finite positive cell spacings and box size.")
    lon = fms.longitude_near(lon,0.5*(dem_meta["lon_min"]+dem_meta["lon_max"]))
    n = int(box_m / m_per_cell) // 2 * 2
    if n < 3:
        raise ValueError("Registration box needs at least three cells per side.")
    ...
    best = None
    for az in azimuths:
        shade = hillshade(heights, (m_per_cell, east_per_cell), az, sun_elevation_deg)
        dr, dc, peak = phase_correlate(gray, shade)
        north, east = -dr * m_per_cell, dc * east_per_cell
...
def against_trek(source, cache_dirs, key, zoom, lat, lon, sizes):
    ...
    for n in sizes:
        ...
        for corrected in (True, False):
            ...
            north, east = -dr * m_per_px, dc * m_per_px*math.cos(math.radians(lat))
