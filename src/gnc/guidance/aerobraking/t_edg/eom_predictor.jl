...
function asim_ctrl_targeting_plot(ip, m, time_0, OE, args, hf, vf, γf, energy_f, v_E, k_cf, heat_rate_control, gram_atmosphere=nothing; cnf=nothing, solution=nothing)
    ...
    function f_ctrl!(y_dot, in_cond, param, t0)
        ...
        #         elseif ip.cm == 1
        #             _control_solarpanels_heatrate(ip, m, args, [1,0], [T_p, ρ, S], t0 - config.cnf.time_IEI, config.cnf.initial_position_closed_form)
        #         elseif ip.cm == 0
        ...
        #             elseif ip.cm == 1
        #                 config.cnf.α = _control_solarpanels_heatrate(ip, m, args, [1,1], config.cnf.state_flesh1[1], t0 - config.cnf.time_IEI, config.cnf.initial_position_closed_form, OE)
        #             elseif ip.cm == 0
        ...
        #                 elseif ip.cm == 1
        #                     config.cnf.α = _control_solarpanels_heatrate(ip, m, args, [1,1], state, t0 - config.cnf.time_IEI, config.cnf.initial_position_closed_form, OE)
        #                 elseif ip.cm == 0
    ...
    function shooting_residual!(resid, z, p, param)
        ...
        #     state = [T_p, ρ, S]
        #     index_ratio = [1]
        #     aoa_hr = _control_solarpanels_heatrate(ip, m, args, index_ratio, state)
