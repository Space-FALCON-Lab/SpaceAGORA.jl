...
function calculate_topography_harmonics!(Clm::AbstractArray{Float64}, Slm::AbstractArray{Float64}, latitude::Float64, longitude::Float64, Plm::AbstractArray{Float64}, topo_degree::Integer, topo_order::Integer)
    ...
    cosmλ = MVector{m+1, Float64}(zeros(m+1))
    cosmλ[1] = 1.0
    if m+1 >= 2
        sinmλ[2] = sin(λ)
        cosmλ[2] = cos(λ)
        constant_cosmλ = 2.0 * cos(λ)
...
function Earth_elevation!(args, Clm::AbstractArray{Float64}, Slm::AbstractArray{Float64}, latitude::Float64, longitude::Float64, A::AbstractArray{Float64})
    """
