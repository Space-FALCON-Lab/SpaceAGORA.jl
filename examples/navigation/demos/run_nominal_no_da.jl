...
mkpath(demo_parent)
demo_output_root = mktempdir(demo_parent; prefix="no_da_", cleanup=false)
...
results_directory = joinpath(demo_output_root, "no_da")
plots_directory = joinpath(demo_output_root, "plots")
animation_path = joinpath(demo_output_root, "animation", "no_da.gif")
...
println(
    ...
    "--mode=plots --case=no-da --input=\"$(results_directory)\" " *
...
println(
    "julia --project=\"$(repo_root)\" \"$(analysis_script)\" " *
    "--mode=animation --case=no-da --input=\"$(results_directory)\" " *
