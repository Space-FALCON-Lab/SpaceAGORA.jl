...
struct YData;
typedef struct YData YelleData;

//! \brief The Yelle model of a Titan Atmosphere.
//!
//! The Yelle model is based on the MinMaxModel.
//! The min-avg-max models are based on observations from Voyager 1 radio science,
//! infrared interferometer - spectrometer(IRIS), and ultraviolet spectrometer
//! (UVS).
//! \ingroup TitanGRAM
class Yelle : public MinMaxModel, public TitanCommon
{
public:
  Yelle();
  Yelle(const Yelle& orig);
  virtual ~Yelle() override = default;
...
private:
  ...
  static const int heightSize = 416;          //!< \brief The number of model data records.
  static const YelleData minData[heightSize]; //!< \brief Model data in table format.
  static const YelleData avgData[heightSize]; //!< \brief Model data in table format.
  static const YelleData maxData[heightSize]; //!< \brief Model data in table format.
  ...
  FRIEND_TEST(Yelle, updateWinds);
  FRIEND_TEST(Yelle, updateMinMaxFactor);
  FRIEND_TEST(Yelle, setMinMaxFactor);
