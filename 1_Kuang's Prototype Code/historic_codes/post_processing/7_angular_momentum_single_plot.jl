# Single plot — Angular Momentum Fractional Drift ΔH/H₀
...
main_xlabel         = "t (hr)";  main_ylabel = "ΔH/H₀  [×10⁻¹¹]"
...
isempty(all_ΔH) && error("Failed to load any CSVs.")

t_hr    = t_common ./ 3600.0
mat     = hcat(all_ΔH...)
mean_ΔH = vec(mean(mat, dims=2))
std_ΔH  = vec(std(mat,  dims=2))
...
fig = plot(legend=false, grid=true, tick_direction=:out,
           xlabel="t (hr)", ylabel="ΔH/H₀  [×10⁻¹¹]",
           title="Angular Momentum Fractional Drift",
           titlefontsize=title_fontsize,
           top_margin=title_top_margin_mm*Plots.mm)
for c in all_ΔH
    plot!(fig, t_hr, c .* 1e11, color=:grey, alpha=0.4, lw=0.5, label="")
end
plot!(fig, t_hr, (mean_ΔH .+ std_ΔH) .* 1e11,
      fillrange=(mean_ΔH .- std_ΔH) .* 1e11,
      fillalpha=0.2, fillcolor=:blue, linealpha=0, label="")
plot!(fig, t_hr, mean_ΔH .* 1e11, color=:blue, lw=1.5, label="")
_yl = ylims(fig)
...
mkpath(output_dir)
out_path = joinpath(output_dir, @sprintf("am_fractional_h%dkm_i%.1fdeg.png", h, i_deg))
