...
| `src/dust.js` | regolith blown off the surface by a descent engine, from the `frames.plume` block: a ground-hugging sheet of advected noise, sparse ejecta points, a scour crater, a haze bank that builds up and thins out, and the sheet's shadow; all sunlit through a Henyey-Greenstein phase function. The sheet's radius, its elevation band, the scour radius and the haze's reach are model outputs (eroding radius, ejection angle, crater radius, deposition radius), not constants |
