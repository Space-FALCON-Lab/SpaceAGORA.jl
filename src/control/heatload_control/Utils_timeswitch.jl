...
function lambdas(m, aoa, k, t, h, γ, v, coeff)
    ...
    for ii in range(start=length(t), step=-1, stop=1)
        lambdav_dot = -3 * k * ρ[ii - 1] * v[ii - 1]^2 * aoa[ii - 1] / pi + lambdav[ii] * (ρ[ii - 1] * S_ref * (CD_0 + aoa[ii - 1] * CD_slope) * v[ii - 1]) / mass - lambdag[ii] * ((ρ[ii - 1] * S_ref * CL_0) / (2 * mass) + g[ii - 1] / v[ii - 1]^2 + 1 / (Rp + h[ii - 1])) - lambdah[ii] * gamma[ii - 1]
        lambdag_dot = lambdav[ii] * g[ii - 1] - lambdah[ii] * v[ii - 1]
        lambdah_dot = k * ρ[ii - 1] * v[ii - 1]^3 * aoa[ii - 1] / (pi * H) - lambdav[ii] * ((ρ[ii - 1] * S_ref * (CD_0 + aoa[ii - 1] * CD_slope) * v[ii - 1]^2) / (2 * mass * H) + 2 * g[ii - 1] * gamma[ii - 1] / (Rp + h[ii - 1])) + lambdag[ii] * (ρ[ii - 1] * S_ref * CL_0 * v[ii - 1] / (2 * mass * H) - 2 * g[ii - 1] / ((Rp + h[ii - 1]) * v[ii - 1]) + v[ii - 1] / (Rp + h[ii - 1])^2)
...
function func(k_cf, m, args, coeff, position, heat_rate_control, approx_sol, aoa_cf, initial_guess=false, approx_calc=false)
    ...
    while temp_Q > 1e-3 # stop if two following trajectory evaluation provides the same Q
        ...
        t_cf, h_cf, γ_cf, v_cf = closed_form(args, m, position, T, m.aerodynamics.α, true, aoa_cf)  # re-evaluate the closed form solution using previous angle of attack profile
