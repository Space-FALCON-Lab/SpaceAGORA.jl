...
Base.@kwdef struct OracleCase2Options
    ...
    output_interval_s::Float64 = 10.0
...
function _usage()
    ...
    --output-dir PATH        Output root; scenarios go into feather/ or smoke/feather/
    --timeseries-points N    Buffer size; trajectory output defaults to every 10 seconds
    --smoke                 Run 60 simulated seconds; keep artifacts under output/smoke
      --animate             Show 3D animation after the simulation (requires GLMakie)
    """
end

# 7. group the command-line option names by types
const _FLAG_OPTS   = (:paper_grid, :feather_only, :animate, :smoke)
...
function main(argv=ARGS)
    ...
    if opts.smoke
        opts.paper_grid && throw(ArgumentError("--smoke cannot be combined with --paper-grid"))
        planet = make_no_gram_planet(:earth)
        period = 2pi * sqrt((planet.Rp_e + opts.target_altitude_km*1e3)^3 / planet.μ)
        opts = _with(opts; orbits=60.0/period, animate=false)
    end
    ...
    if opts.paper_grid
        ...
        (opts.feather_only || opts.smoke) || plot_open_cavity_results(result, opts; IMG_DIR=img_dir, target_only=false) # step 4: generate diagnostic plots (unless --feather-only is specified, then don't plot)
