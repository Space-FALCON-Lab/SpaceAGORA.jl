...
# impulse tracker (stored in p[:tracker]) for the configured target satellite.
...
function a_gravity_laser_RTN_time_series(sol::_FlatSol, p::Dict)
    ...
    if !isnothing(tracker) && length(tracker.t_hist) >= 2
        ...
        for k_tr in 2:length(th)
            ...
            a_laser_RTN_hist[target_id][:,k_sol] .+= [a_R, a_T, a_N]
        end
    end
...
function delta_v_RTN_time_series(sol::_FlatSol, p::Dict)
    ...
    Δv_RTN_hist[target_id][1,:] = tracker.dv_R_hist
    Δv_RTN_hist[target_id][2,:] = tracker.dv_T_hist
    Δv_RTN_hist[target_id][3,:] = tracker.dv_N_hist
...
function laser_force_RTN_time_series(sol::_FlatSol, p::Dict)
    ...
    for k in 2:nT_tr
        dt = th[k] - th[k-1]
        dt <= 0.0 && continue
        F_R = target_mass * (tracker.dv_R_hist[k] - tracker.dv_R_hist[k-1]) / dt
        F_T = target_mass * (tracker.dv_T_hist[k] - tracker.dv_T_hist[k-1]) / dt
        F_N = target_mass * (tracker.dv_N_hist[k] - tracker.dv_N_hist[k-1]) / dt
        k_sol = clamp(searchsortedlast(t_flat, th[k]), 1, nT_flat)
        F_RTN_hist[target_id][:,k_sol] .+= [F_R, F_T, F_N]
    end
    # Cumulative ΔP at each flat sol time by interpolation from tracker
    for k in 1:nT_flat
        k_tr = clamp(searchsortedlast(th, t_flat[k]), 1, nT_tr)
        ΔP_RTN_hist[target_id][:,k] = [target_mass*tracker.dv_R_hist[k_tr],
                         target_mass*tracker.dv_T_hist[k_tr],
                         target_mass*tracker.dv_N_hist[k_tr]]
    end
...
# =============================================================================
# Get target r/v; the target is last in ORACLE solutions.
@inline function _target_rv_at(sol, t::Float64; target_id::Int=length(first(sol.u).sc))
