...
p1 = plot(legend=false, grid=true, tick_direction=:out,
          ...
          title="Normalized Drift of Linear Momentum (P), Angular Momentum (H) and Orbital Energy (E)",
...
_draw_panel!(p2, x_axis, all_ΔH, mean_ΔH, std_ΔH; scale=1e11)

# ── Panel 3: Orbital Energy ───────────────────────────────────────────────────
p3 = plot(legend=false, grid=true, tick_direction=:out,
          xlabel="Number of orbits (target satellite)", ylabel="ΔE/|E₀|  [×10⁻⁶]",
          xguidefontsize=xlabel_fontsize, yguidefontsize=ylabel_fontsize,
          #title="Orbital Energy Fractional Drift",
          titlefontsize=title_fontsize,
          top_margin=panel_top_margin_mm*Plots.mm)
_draw_panel!(p3, x_axis, all_ΔE, mean_ΔE, std_ΔE; scale=1e6)
