...
main_bus = config.Link(root=true, 
                        ...
                        dims=SVector{3, Float64}([0.2,0.1,0.3]), 
                        ref_area=0.3*0.1,
                        m=12)
Lpanel = config.Link(root=false, 
                        r=SVector{3, Float64}(0.0, 0.4, 0.0), 
                        ṙ=SVector{3, Float64}([0,0,0]),
                        q=SVector{4, Float64}([0.0,0.38268,0.0,0.92387]), 
                        dims=SVector{3, Float64}([0.01,0.7,0.3]), 
                        ref_area=0.7*0.3,
                        m=0.0)
Rpanel = config.Link(root=false, 
                        r=SVector{3, Float64}(0.0, 0.4, 0.0), 
                        ṙ=SVector{3, Float64}([0,0,0]),
                        q=SVector{4, Float64}([0.0,0.38268,0.0,0.92387]), 
                        dims=SVector{3, Float64}([0.01,0.7,0.3]), 
...
config.add_body!(spacecraft, Lpanel)
config.add_body!(spacecraft, Rpanel)

# Define joints
Lpanel_joint = config.Joint(link1=main_bus, link2=Lpanel, p1=SVector{3, Float64}(0.0, -0.05, 0.0), p2=SVector{3, Float64}(0.0, 0.35, 0.0), rotational_displacement=SVector{4, Float64}(0.0,0.38268,0.0,0.92387))
Rpanel_joint = config.Joint(link1=main_bus, link2=Rpanel, p1=SVector{3, Float64}(0.0, 0.05, 0.0), p2=SVector{3, Float64}(0.0, -0.35, 0.0), rotational_displacement=SVector{4, Float64}(0.0,-0.38268,0.0,0.92387))
config.add_joint!(spacecraft, Lpanel_joint)
config.add_joint!(spacecraft, Rpanel_joint)
...
args = Dict(# Misc Simulation
            ...
            :directory_results => "output/earth_ptdr",            # Directory where to save the results
            ...
            :save_csv => false,                                        # Save results in csv files True=1, False=0
            ...
            :ra_initial_a => 28523.95e3 + 6.0518e6, # 66597e3,                # Initial Apoapsis Radius for for-loop in m
            :ra_initial_b => 1e21,                               # Final Apoapsis Radius for for-loop in m
            :ra_step => 5e21,                                       # Step Apoapsis Radius for for-loop in m
            :hp_initial_a => 200600.0,#176590.0,#188140.0                                 # Initial Periapsis Altitude for for-loop in m
            ...
            :a_initial_a => 6770.24e3,                         # Initial semi-mjaor axis (km) for for-loop if initial conditions are in keplerian elements
            :a_initial_b => 31453.79499727e3,                         # Final semi-major axis (km) for for-loop if initial conditions are in keplerian elements
            :a_step => 1e10,                                         # Step semi-major axis (km) for for-loop if initial conditions are in keplerian elements
            :e_initial_a => 0.0003355,                                  # Initial eccentricity for for-loop if initial conditions are in keplerian elements
            :e_initial_b => 0.08,                                  # Final eccentricity for for-loop if initial conditions are in keplerian elements
            :e_step => 0.1,                                         # Step eccentricity for for-loop if initial conditions are in keplerian elements
            ...
            :inclination => 97.3868,                                   # Inclination Orbit, deg
            :ω => 74.9344,                                              # AOP, deg
            :Ω => 111.3651,                                              # RAAN, deg
            :ν => 285.2285,                                             # True Anomaly, deg
            ...
            # Final Conditions
            :final_apoapsis => 0.0, # 4905.974818462152e3                  # Final apoapsis radius if aerobraking campaign
            ...
            :maneuver_plan => Earth_firing_plan,          # MAneuver plan function  
