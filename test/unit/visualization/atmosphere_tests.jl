...
@testset "Atmosphere spec" begin
    ...
    @testset "spatially varying model gets a map" begin
        ...
        # With them, both the profile and the map are sampled on the requested grid.
        spec = atmosphere_spec(args; density_params=(dummy=true,), profile_points=8, map_step_deg=30.0)
        ...
        @test length(d["atmosphere"]["map"]["density_kg_m3"]) == length(d["atmosphere"]["map"]["lat_deg"]) * length(d["atmosphere"]["map"]["lon_deg"])
    end
end
