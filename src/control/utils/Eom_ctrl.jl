...
function asim_ctrl_plot(ip, m, time_0, OE, args, v_E, k_cf, heat_rate_control, gram_atmosphere=nothing)
    ...
    function f_ctrl!(y_dot, in_cond, param, t0)
        ...
        lambda_switch = (2.0 * mass * vel_ii_mag) / (area_tot * CD_slope * pi)
        ...
        if args[:srp] == true
            ...
            srp_ii = mass * srp(m.planet, p_srp_unscaled, m.aerodynamics.reflection_coefficient, m.body.area_tot, mass, pos_ii, et)
        end
    ...
    lambda_switch_list = (2.0 * mass * v_ii_mag) ./ (area_tot * CD_slope * pi)
...
function asim_ctrl_rf(ip, m, time_0, OE, args, v_E, k_cf, heat_rate_control, gram_atmosphere=nothing)
    ...
    function time_switch_func_condition(y, t, integrator)
        ...
        mass = config.get_spacecraft_mass(m.body)              
        area_tot = config.get_spacecraft_reference_area(m.body)
