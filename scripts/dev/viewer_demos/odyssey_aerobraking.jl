...
#
# The optional accelerometer mode uses local per-pass telemetry density.
# Differences from a navigation reconstruction are model-comparison diagnostics,
# not an established atmospheric bias or out-of-sample validation claim.
#
#   julia --project=. scripts/dev/viewer_demos/odyssey_aerobraking.jl
include(joinpath(@__DIR__, "common.jl"))
function _mission_main(argv)

atmosphere_mode = get(ENV, "SPACEAGORA_DEMO_ODYSSEY_ATMOSPHERE", "gram")
atmosphere_mode in ("gram", "accelerometer") || throw(ArgumentError("atmosphere option must be gram or accelerometer"))
ACCELEROMETER = atmosphere_mode == "accelerometer"
case_name = demo_case_name("odyssey_aerobraking") * (ACCELEROMETER ? "_accelerometer" : "")
DENSITY_TABLE = joinpath(REPO_ROOT, "data", "telemetry", "Odyssey", "odyssey_accelerometer_density.feather")
MODEL = demo_model_path("mars_odyssey_nasa_3d_resources.glb")
# array normal (model Z) along the flow with the boom trailing (model +Z to body -x); array offset (model +Y) to body +y
ODYSSEY_ROTATION_DEG = (0, -90, 0)
...
density_model = if ACCELEROMETER
    ...
    GRAMAtmosphereModel(planet_name="mars", initial_time=initial_time, mars_map_year=2)
end
...
sc = make_three_body_spacecraft(
    ...
    ic=cartesian_ic_at(ody, et0), reflection_coefficient=0.9, prop_mass=50.0, id=1)
effectors = (
...
html = export_visualization(prefix; max_frames=5000, trail_orbits=1, texture_resolution="4k",
    title="AGORA Odyssey · aerobraking at Mars, modeled trajectory with the SPICE reference" * (ACCELEROMETER ? ", accelerometer density" : "") * (mesh ? " (mesh aerodynamics)" : ""),
...
cdn = build_cdn_page(html, joinpath(OUTDIR, "artifact.html"), "AGORA Odyssey Aerobraking",
    "AGORA Odyssey · aerobraking at Mars, from 2001-11-06",
    ...
    "$(round(mission_time / 3600; digits=3)) h modeled span",
