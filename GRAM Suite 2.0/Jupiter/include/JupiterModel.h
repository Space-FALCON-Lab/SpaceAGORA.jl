...
struct JupData;
typedef struct JupData JupiterModelData;

//! \brief The height based model of a Jupiter Atmosphere.
//!
//! The JupiterModel utilizes the simple height based data model.
//! The data for the model was provided by ???.
//! \ingroup JupiterGRAM
class JupiterModel : public HeightModel, public JupiterCommon
{
public:
  JupiterModel();
  JupiterModel(const JupiterModel& orig);
  virtual ~JupiterModel() override = default;
...
private:
  ...
  static const size_t heightSize = 693;           //!< The number of model data records.
  static const JupiterModelData data[heightSize]; //!< Model data in table format.
