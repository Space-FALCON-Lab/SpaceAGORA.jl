...
draws it with the SpaceAGORA viewer, with no Julia and no simulation.
  ...
  python3 viewer/build_standalone.py --out page.html --data run.csv --planet earth --epoch 2020-01-01T00:00:00Z \
      --model data/models/iss_nasa_3d_resources_b.glb --model-scale 2.4 --model-rotation=-90,0,-90 --title "ISS"
...
MODULES = ["data.js", "colormaps.js", "timeline.js", "globe.js", "atmosphere.js", "spacecraft.js", "lod.js", "ensemble.js", "paths.js", "references.js", "groundtrack.js", "video.js", "plots.js", "terrain.js", "plumes.js", "dust.js", "lighting.js", "ui.js", "main.js", "standalone.js"]
VENDOR = {
...
def _texture_manifest(text):
    if tomllib is not None:
        return tomllib.loads(text)
    # The shipped manifest uses [[texture]] tables with JSON-compatible
    # scalar values. Do not silently guess at unsupported TOML syntax.
    entries = []
    current = None
    for number, raw in enumerate(text.splitlines(), 1):
        ...
        if line == "[[texture]]":
            ...
            continue
        if current is None and line.startswith("version ="):
            continue
        key, separator, value = line.partition("=")
        if current is None or not separator or not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", key.strip()):
            raise ValueError(f"Unsupported texture manifest syntax on line {number}; use Python 3.11+ for full TOML support.")
        try:
            parsed = json.loads(value.strip())
        except json.JSONDecodeError as error:
            raise ValueError(f"Unsupported texture manifest value on line {number}; use Python 3.11+ for full TOML support.") from error
        if not isinstance(parsed, (str, int, float, bool)):
            raise ValueError(f"Expected a scalar texture value on line {number}.")
        current[key.strip()] = parsed
    return {"texture": entries}


def texture_table(tier, bodies=None):
    manifest = _texture_manifest((TEXTURES / "manifest.toml").read_text())
    out = {}
