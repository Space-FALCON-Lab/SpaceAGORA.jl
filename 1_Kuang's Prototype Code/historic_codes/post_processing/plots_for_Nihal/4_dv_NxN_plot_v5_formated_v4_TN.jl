...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        # Per-altitude filter list: h=1000 CSV has 249 helpers instead of 250
        filter_helpers = h == 1000 ? sort(unique([1, 50, 150, 200, 249, 300])) :
                                     sort(unique([1, 50, 150, 200, 250, 300]))

        # Filter first (on raw CSV values, so 249 is still 249)
        ...
        # For h=1000: remap 249 → 250 in display labels (after filtering)
        ...
        if res === nothing
            ...
            let all_vals = vcat(dv_final_T, dv_final_N)
                dv_max = maximum(all_vals); dv_min = minimum(all_vals)
                half_span = max((dv_max - dv_min) / 2, abs(dv_max) * 0.5, 1e-9)
                global y_lo = dv_min - half_span
                global y_hi = dv_max + half_span
            end
            ...
            sp = plot(legend=false,
                      ...
                      left_margin=gap_inner*Plots.mm, right_margin=gap_inner*Plots.mm,
                      top_margin=gap_inner*Plots.mm, bottom_margin=gap_inner*Plots.mm)
            plot!(sp, xs, dv_final_T;
                  label=L"\Delta v_T", color=:orange, linestyle=:solid, lw=1.5, marker=:circle, ms=3)
            plot!(sp, xs, dv_final_N;
                  label=L"\Delta v_N", color=:green,  linestyle=:solid, lw=1.5, marker=:square, ms=3)
        end
...
# Legend strip — manually drawn so entries are never clipped
legend_sp = plot(framestyle=:none, ticks=nothing, legend=false, grid=false,
                 xlims=(0,1), ylims=(0,1))
# Entry 1: Δv_T (orange solid line + label)
plot!(legend_sp, [0.30, 0.38], [0.5, 0.5]; color=:orange, linestyle=:solid, lw=1.5)
scatter!(legend_sp, [0.34], [0.5]; color=:orange, marker=:circle, ms=3, label=false)
annotate!(legend_sp, 0.40, 0.5, text(L"\Delta v_T", :black, :left, legend_fontsize))
# Entry 2: Δv_N (green solid line + label)
plot!(legend_sp, [0.55, 0.63], [0.5, 0.5]; color=:green, linestyle=:solid, lw=1.5)
scatter!(legend_sp, [0.59], [0.5]; color=:green, marker=:square, ms=3, label=false)
annotate!(legend_sp, 0.65, 0.5, text(L"\Delta v_N", :black, :left, legend_fontsize))
corner_leg_sp = plot(framestyle=:none, ticks=nothing, legend=false, grid=false)

# 2-row macro layout: [ y-strip | NxN grid ]
#                     [ corner  | x-strip  ]
l = @layout [a{0.06w} grid(n_rows, n_cols) ; b{0.001w} c{0.05h}] # change gap around main xlabel and main ylabel here !!!!!
