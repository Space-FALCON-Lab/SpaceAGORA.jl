# NxN grid of dΔv/dt vs Number of Helpers subplots.
# Rows = altitudes, Columns = inclinations.
# Each subplot shows dΔv_R/dt, dΔv_T/dt, dΔv_N/dt vs number of helper satellites.
...
main_xlabel          = "Number of helpers"       # shared x-axis label
 main_ylabel          = "dΔv/dt  [×10⁻⁵ m/s²]"  # shared y-axis label
...
scale_factor         = 1e5 # multiply slope data by this; update main_ylabel to match
    ...
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        if res !== nothing && !isempty(filter_helpers)
            ...
            res  = (N_helpers_vec = res.N_helpers_vec[mask],
                    slope_R       = res.slope_R[mask],
                    slope_T       = res.slope_T[mask],
                    slope_N       = res.slope_N[mask])
        end
...
fig = plot(yl_sp, subplots..., corner_sp, xl_sp, corner_leg_sp, legend_sp,
           ...
           plot_title="Rate of Δv in RTN vs Number of Helper Satellites",
...
mkpath(output_dir)
out_path = joinpath(output_dir, "dv_NxN_plot.png")
