...
9. Structural analysis ownership is `src/vehicle/structure/*`, not `src/vehicle/spacecraft/*`; CAD/mesh readers live in `src/vehicle/structure/mesh_geometry.jl` and the free-molecular panel method and its surrogate effector in `src/dynamics/coupled/aerodynamic_mesh_surrogate.jl` (coupled dynamics, like the other aerodynamic effectors).
10. Visualization scene ownership is `src/analysis/visualization/scene/*` (a canonical owner inside `SimulationModel`); the browser viewer is top-level `viewer/*`, and `src/viewer/*` is forbidden.
...
6. `test/gates/ci_path_policy_gate.jl`
7. `test/gates/ci_vehicle_structure_boundary_gate.jl`
8. `test/gates/ci_gnc_aerobraking_boundary_gate.jl`
9. `test/gates/ci_no_guidance_control_cross_include_gate.jl`
10. `test/gates/ci_gnc_typed_command_boundary_gate.jl`
11. `test/gates/ci_aerobraking_strategy_contract_gate.jl`
