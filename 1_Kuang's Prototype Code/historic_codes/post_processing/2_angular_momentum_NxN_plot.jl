# NxN grid of Angular Momentum Fractional Drift subplots.
# Rows = altitudes, Columns = inclinations.
# Each subplot calls plot_angular_momentum(h, i) for data; blank with "No Data" if unavailable.

include(joinpath(@__DIR__, "csv_plotter_functions", "5_angular_momentum_function.jl"))
...
main_ylabel          = "ΔL/L₀  [×10⁻¹¹]"  # shared y-axis label
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        local sp
        local res = try
            plot_angular_momentum(h, i_deg; show_variance=true)
        catch
            nothing
        end
        ...
        if res === nothing
            ...
            for ΔH_curve in res.all_ΔH
                plot!(sp, t_hr, ΔH_curve .* 1e11, color=:grey, alpha=0.4, lw=0.5, label="")
            end
            # ±1σ band
            plot!(sp, t_hr, (res.mean_ΔH .+ res.std_ΔH) .* 1e11,
                  fillrange=(res.mean_ΔH .- res.std_ΔH) .* 1e11,
                  fillalpha=0.2, fillcolor=:blue, linealpha=0, label="")
            # mean line
            plot!(sp, t_hr, res.mean_ΔH .* 1e11, color=:blue, lw=1.2, label="")
        end
...
fig = plot(yl_sp, subplots..., corner_sp, xl_sp,
           ...
           plot_title="Angular Momentum Fractional Drift  ΔL/L₀",
...
mkpath(output_dir)
out_path = joinpath(output_dir, "angular_momentum_NxN_plot.png")
