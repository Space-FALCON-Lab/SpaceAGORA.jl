...
# body, exactly as the LM guidance computer did; the commanded thrust
...
# vehicle reaches the high-gate altitude, as flown.
...
drops to `t_go_min_s` or the radar altitude drops below `altitude_switch_m`.
"""
...
"""
    ApolloDescentConfig(; site_lat_deg, site_lon_deg, approach_azimuth_deg, braking, approach, ...)

Landing site (planetocentric degrees, east longitude; `site_height_m` above
...
it commands full thrust, as the DPS did.
"""
...
attitude (scalar-last, body to inertial) and body rate, the radar altitude
...
the radar altitude from the terrain model, and writes the thrust and
attitude commands into `state` for [`ApolloDescentControlModel`](@ref).
"""
struct ApolloDescentGuidanceModel{T <: AbstractTerrainModel} <: AbstractGuidanceModel
    config::ApolloDescentConfig
    state::ApolloDescentState
    terrain::T
end
ApolloDescentGuidanceModel(config::ApolloDescentConfig, state::ApolloDescentState) = ApolloDescentGuidanceModel(config, state, NoTerrainModel())
...
function descent_site_frame(config::ApolloDescentConfig, terrain::AbstractTerrainModel, reference_radius_m::Real)
    ...
    az = deg2rad(config.approach_azimuth_deg)
...
function quaternion_from_passive_dcm(A::SMatrix{3, 3, Float64})::SVector{4, Float64}
    # rot(q) = R(q)' with R the active matrix; read the active matrix R = A'
    R = A'
    t = R[1, 1] + R[2, 2] + R[3, 3]
    if t > 0.0
        s = sqrt(t + 1.0) * 2
        q = SVector{4, Float64}((R[3, 2] - R[2, 3]) / s, (R[1, 3] - R[3, 1]) / s, (R[2, 1] - R[1, 2]) / s, 0.25 * s)
    elseif R[1, 1] > R[2, 2] && R[1, 1] > R[3, 3]
        s = sqrt(1.0 + R[1, 1] - R[2, 2] - R[3, 3]) * 2
        q = SVector{4, Float64}(0.25 * s, (R[1, 2] + R[2, 1]) / s, (R[1, 3] + R[3, 1]) / s, (R[3, 2] - R[2, 3]) / s)
    elseif R[2, 2] > R[3, 3]
        s = sqrt(1.0 + R[2, 2] - R[1, 1] - R[3, 3]) * 2
        q = SVector{4, Float64}((R[1, 2] + R[2, 1]) / s, 0.25 * s, (R[2, 3] + R[3, 2]) / s, (R[1, 3] - R[3, 1]) / s)
    else
        s = sqrt(1.0 + R[3, 3] - R[1, 1] - R[2, 2]) * 2
        q = SVector{4, Float64}((R[1, 3] + R[3, 1]) / s, (R[2, 3] + R[3, 2]) / s, 0.25 * s, (R[2, 1] - R[1, 2]) / s)
    end
    return q / norm(q)
end
...
Planet-fixed and site-frame state of spacecraft `i`, its radar altitude
...
function descent_environment(model::ApolloDescentGuidanceModel, u, p::ODEParams, t::Float64, i::Int)
    ...
    if frame === nothing
        frame = descent_site_frame(model.config, model.terrain, planet.Rp_e)
        model.state.site_frame[i] = frame
    end
    lla = rtolatlong(r_p, planet)
    lat_deg = rad2deg(lla[2]); lon_deg = rad2deg(lla[3])
    h_ground = terrain_height(model.terrain, lat_deg, lon_deg)
    radar = norm(r_p) - frame.reference_radius_m - h_ground
...
function calcGuidanceEffect!(model::ApolloDescentGuidanceModel, u, p::ODEParams, t::Float64, i::Int64)
    state = model.state
    config = model.config
    1 <= i <= length(state.phase) || return nothing
