# NxN grid of final RAAN vs number of helper satellites.
# Rows = altitudes, Columns = inclinations.
# Each subplot shows Ω(end) for every helper count in filter_helpers.
...
# When span==0 (all identical), generates n symmetric ticks centered on the value.
function nice_ticks(lo, hi; n=5)
    span = hi - lo
    if span == 0
        # All values identical — n ticks centered on lo (middle tick = lo)
        ctr  = lo
        half_default = abs(ctr) > 0 ? abs(ctr) * 0.1 : 1.0
        mag0 = 10.0^floor(log10(half_default))
        for mult in (1.0, 2.0, 5.0, 10.0, 20.0, 50.0)
            step = mult * mag0
            step >= half_default || continue
            half = (n ÷ 2) * step   # for n=5: 2 steps either side
            vals = [ctr - half + k * step for k in 0:(n-1)]
...
main_ylabel          = L"\Delta\Omega,\ \mathrm{deg}"
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        local res = try
            plot_OE(h, i_deg; element=:Ω, show_variance=false,
        ...
        if res === nothing
            ...
            # Final RAAN for each helper count — sort ascending
            N_helpers_vec  = res.N_vals .- 1
            if h == 1000; N_helpers_vec = [n == 249 ? 250 : n for n in N_helpers_vec]; end
            raan_final_deg = [v[end] - v[1] for v in res.all_vals]  # ΔΩ in degrees (plot_OE already converts)
            ord            = sortperm(N_helpers_vec)
            N_helpers_vec  = N_helpers_vec[ord]
            raan_final_deg = raan_final_deg[ord]
            # Snap floating-point noise to zero; use ±20 deg range for zero data
            _is_zero = maximum(abs.(raan_final_deg)) < 1e-6
            if _is_zero; fill!(raan_final_deg, 0.0); end
            xs             = N_helpers_vec             # actual values → proper spacing

            _rmin = minimum(raan_final_deg); _rmax = maximum(raan_final_deg)
            _rctr  = (_rmin + _rmax) / 2
            _rhalf = max((_rmax - _rmin) / 2 * 1.1, abs(_rctr) * 0.5, 1e-9)
            ytk_vals, ytk_labs = _is_zero ? nice_ticks(-20.0, 20.0; n=5) :
                                            nice_ticks(_rctr - _rhalf, _rctr + _rhalf; n=5)
            ...
            sp = plot(legend=false, grid=true,
                      ...
                      xrotation=45,
                      ylims=(ytk_vals[1], ytk_vals[end] + ytk_step),
                      yticks=(ytk_vals, ytk_labs),
                      left_margin=gap_inner*Plots.mm, right_margin=gap_inner*Plots.mm,
                      top_margin=gap_inner*Plots.mm, bottom_margin=gap_inner*Plots.mm)
                plot!(sp, xs, raan_final_deg;
...
annotate!(legend_sp, 0.45, 0.5,
          text(L"\Delta\Omega", :black, :left, legend_fontsize))
...
mkpath(output_dir)
out_path     = joinpath(output_dir, "OE_raan_final_NxN_plot.png")
out_path_pdf = joinpath(output_dir, "OE_raan_final_NxN_plot.pdf")
