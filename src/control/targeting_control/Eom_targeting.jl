...
function asim_ctrl_targeting_plot(ip, m, time_0, OE, args, hf, vf, γf, k_cf, heat_rate_control, gram_atmosphere=nothing)
    ...
    function shooting_residual!(resid, z, p, param)
        ...
        # resid[5] = lambdagamma_fin - (0.0 + p2)
    ...
    z0 = [4300, 0.0, 160000] # , -10.0, -10.0, -10.0, 600]
    # z0 = [4300, 0.0, 160000]
    ...
    sol_NL = nlsolve((residuals, z) -> shooting_residual!(residuals, z, p, param), z0, method = :newton)
    ...
    # # lambdav = (v_cf[end]^2/2 - m.planet.μ / (m.planet.Rp_e + h_cf[end])^2 - vf^2/2 + m.planet.μ / (m.planet.Rp_e + hf)^2) * v_cf[end]
    # # lambdag = 0.0
    # # lambdah = (v_cf[end]^2/2 - m.planet.μ / (m.planet.Rp_e + h_cf[end])^2 - vf^2/2 + m.planet.μ / (m.planet.Rp_e + hf)^2) * m.planet.μ / (m.planet.Rp_e + h_cf[end])^2

    # lambdav = 4196  # vf
    # lambdag = 0
    # lambdah = 10000000 # m.planet.μ / (m.planet.Rp_e + hf)^2
    ...
    #     v_fin = norm(sol[4:6,end])
    #     Q_fin = sol[end,end]
    #     lambda_v_fin = (v_fin^2/2 - m.planet.μ / r_fin^2 - vf^2/2 + m.planet.μ / (m.planet.Rp_e + hf)^2) * v_fin
    #     lambda_γ_fin = 0.0
    #     lambda_h_fin = (v_fin^2/2 - m.planet.μ / r_fin^2 - vf^2/2 + m.planet.μ / (m.planet.Rp_e + hf)^2) * m.planet.μ / r_fin^2
    ...
    #     if (abs(lambda_v_fin_actual - lambda_v_fin) < 0.1 && abs(lambda_γ_fin_actual - lambda_γ_fin) < 0.1 && abs(lambda_h_fin_actual - lambda_h_fin) < 0.01) || count > 97
    #         break
    #     end
        
    #     println("diff: ", [abs(lambda_v_fin_actual - lambda_v_fin), abs(lambda_γ_fin_actual - lambda_γ_fin), abs(lambda_h_fin_actual - lambda_h_fin)])
    ...
    # println("lambdas t_f: ", [sol[7,end], sol[8,end], sol[9,end]])

    # # Initial condition initialization
    # in_cond = [sol[1,end], sol[2,end], sol[3,end], sol[4,end], sol[5,end], sol[6,end], sol[7,end], sol[8,end], sol[9,end], sol[10,end]]
