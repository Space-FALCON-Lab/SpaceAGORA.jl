...
# Uses the Constellation / OraclePattern API added to 4_SpaceAGORA.jl-main.
...
const _FLAG_OPTS_ORACLE   = Symbol[]
const _INT_OPTS_ORACLE    = (:helpers, :timeseries_points)
const _SYMBOL_OPTS_ORACLE = (:schedule,)
const _PATH_OPTS_ORACLE   = (:output_dir,)
const _FLOAT_OPTS_ORACLE  = (
...
function _make_oracle_sc(id::Int, ic, p)
    bus = Link(root=true, m=p.mass_kg)
    return SpacecraftModel(Link[], [bus], bus, true, p.mass_kg, 0.0,
                           bus.inertia, 0, 0, ic, id)
end

# ── ODE state helper (mirrors 2_SpaceAGORA ORACLE helper) ─────────────────────

function _rv_to_elements(r::SVector{3,Float64}, v::SVector{3,Float64}, mu::Float64)
    r_norm = norm(r)
    v2     = dot(v, v)
    a      = -mu / (v2 - 2.0 * mu / r_norm)
    h_vec  = cross(r, v)
    h_norm = norm(h_vec)
    p_slr  = h_norm^2 / mu
    ...
    n_mag  = norm(n_asc)
    raan   = n_mag > 1e-12 ? atan(n_asc[2], n_asc[1]) : 0.0
    return (a=a, e=e, i=i, raan=raan, p=p_slr)
end
...
function run_oracle_laser(opts::OracleLaserOptions)
    ...
    # 1. Build constellation via OraclePattern
    ...
    pattern = OraclePattern(
        target_altitude_m       = target_radius_m,
        helper_altitude_m       = helper_radius_m,
    ...
    # Override target IC with non-zero eccentricity / true anomaly if requested
    function make_sc(id, ic, p)
        if id == 1 && (opts.target_ecc != 0.0 || opts.target_nu_deg != 0.0)
            ic = InitialCondition(
                target_radius_m * (1.0 + opts.target_ecc),  # ra
                target_radius_m * (1.0 - opts.target_ecc),  # rp via apsides
                opts.target_inclination_deg,
                0.0, 0.0,
                opts.target_nu_deg
            )
        end
        return _make_oracle_sc(id, ic, p)
    end

    constellation = Constellation(pattern, make_sc)

    # 2. Laser model (uses positional indices = satellite array position)
    ...
    args = SimulationConfiguration(
        ...
        dynamics_model = DynamicsModel(
            ...
            (InverseSquaredJ2GravityModel(), laser_model)
        ),
    ...
    oe0 = _rv_to_elements(r0, v0, planet.μ)
    ...
    oef = _rv_to_elements(rf, vf, planet.μ)
