...
function _run_flyby(key::String; half_span_s::Float64=2.5 * 3600.0, argv=String[])
    isfinite(half_span_s) && half_span_s > 0 || throw(ArgumentError("half_span_s must be finite and positive"))
    haskey(FLYBYS, key) || throw(ArgumentError("unknown flyby; choose TA or T5"))
    fb = FLYBYS[key]
    options = viewer_demo_options(demo_case_name("cassini_$(lowercase(key))"), 2half_span_s; argv=argv)
    outdir = options.output_dir
    model = demo_model_path(fb.model)
    ...
    initial_time = initial_time_of(et_ca - half_span_s)
    ...
    effectors = (
        GravitationalHarmonicsModel(5, 5, joinpath(HARMONICS_DIR, "titan5.csv"), planet;
            coefficient_normalization=:full, reference_radius_m=2575000.0, gm_m3s2=8978126919238.97),
    ...
    base = make_example_config(planet=planet, spacecraft=sc, mission_time=mission_time, initial_time=initial_time,
        dynamic_effectors=effectors, density_model=GRAMAtmosphereModel(planet_name="titan", initial_time=initial_time), orientation_sim=false,
    ...
    cdn = build_cdn_page(html, joinpath(outdir, "artifact.html"), "AGORA Cassini $(key) Flyby",
        ...
        "$(round(mission_time / 3600; digits=3)) h modeled span; closest approach $(utc_of(et_ca)[1:16])Z",
    ...
    return (args=args, prefix=prefix, html=html, cdn=cdn, et_ca=et_ca)
end

function run_flyby(key::String; kwargs...)
    haskey(FLYBYS, key) || throw(ArgumentError("unknown flyby; choose TA or T5"))
    setup_gram_example!()
    return Base.invokelatest(_run_flyby, key; kwargs...)
end

function main(argv=ARGS)
which = isempty(argv) || startswith(first(argv), "--") ? "TA" : uppercase(first(argv))
options = isempty(argv) || startswith(first(argv), "--") ? argv : argv[2:end]
which == "ALL" && "--output-dir" in options && throw(ArgumentError("all flybys need distinct default output directories; omit --output-dir or run one flyby"))
for key in (which == "ALL" ? ("TA", "T5") : (which,))
    haskey(FLYBYS, key) || throw(ArgumentError("unknown flyby $(key); use TA, T5 or all"))
    run_flyby(key; argv=options)
end
