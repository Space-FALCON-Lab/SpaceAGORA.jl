# Run the targeted phase-dependence grid for inclination x apoapsis slices.
#
# This supplement is designed for four-panel phase plots with argument of
# periapsis fixed at 0, 90, 180, and 270 deg:
# - Mars, Venus, Earth, and Titan
# - nominal periapsis
# - full_environment with nominal density only
# - mass scale 1
# - dense inclination x apoapsis grid
#
# Usage:
...
const PHASE_INCLINATIONS_DEG = sort(unique(vcat(
    ...
    [AerobrakingPerturbationMC.NOMINAL_INCLINATION_DEG],
)))

const PHASE_APOAPSIS_ALTITUDES_KM = Dict{Symbol, Vector{Float64}}(
...
function _parse_supplement_args(args::Vector{String})
    ...
    output_dir = joinpath(AerobrakingPerturbationMC.REPO_ROOT, "output", "aerobraking_perturbation_mc_phase_inclination_apoapsis_supplement")
    resume = false
    resume_dir = ""
    ...
    while i <= length(args)
        ...
        elseif arg in ("-h", "--help")
            println("""
            Usage:
              julia --project=. benchmarks/studies/aerobraking_perturbation_mc/run_phase_inclination_apoapsis_supplement.jl [options]
...
function main(args=ARGS)
    ...
    spec = AerobrakingPerturbationMC.StudySpec(
        ...
        periapsis_regimes=[:nominal],
        apoapsis_altitudes_km=deepcopy(PHASE_APOAPSIS_ALTITUDES_KM),
        dynamics_cases=[:full_environment],
        density_scales=Dict(:nominal => 1.0),
        spacecraft_mass_scales=[1.0],
        inclinations_deg=copy(PHASE_INCLINATIONS_DEG),
        argp_degs=copy(PHASE_ARGP_DEGS),
