...
# Options #
###########
# Select the folder containing trajectory.feather, geometry_encounters.feather,
# and laser_on.feather. The REPL run button uses these settings, not REPL ARGS.
const OPTIONS = (
    bundle_directory = joinpath(@__DIR__, "output", "feather",
        "prototype_N10_h1000km_t1050km_ih0_it0_e0_nu0_T63071s_R200km_P10000W_B100_m227kg_none_J2true_dt10s"),
...
function load_replay_bundle(directory)
    ...
    get(metadata, "source", "") == "Kuang prototype" ||
        throw(ArgumentError("This replay loader expects a Kuang prototype bundle"))
    ...
    target_id = parse(Int, get(metadata, "target_id", "1"))
    ...
    caption_parts = match(r"^prototype_(N\d+_h[^_]+_t[^_]+_ih[^_]+_it[^_]+_e[^_]+_nu[^_]+)_T.*?_m[^_]+kg_(.+)_J2", scenario)
    ...
    return (; sol=ReplayTrajectory(times, states), params, geometry, lasers,
        ...
        earth_radius=parse(Float64, get(metadata, "earth_radius_m", string(R_EARTH))))
end
...
function replay_feather_animation(directory=DEFAULT_BUNDLE;
    duration_seconds=OPTIONS.duration_seconds, animation_fps=OPTIONS.animation_fps,
        output_file=joinpath(dirname(dirname(abspath(directory))), "videos", basename(normpath(directory)), "animation_replay.mp4"),
        render_options...)
