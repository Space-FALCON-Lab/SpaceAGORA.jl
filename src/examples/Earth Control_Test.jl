...
args = Dict(# Misc Simulation
            ...
            :directory_results => "output/Earth_switching_analysis_targeting_cf",    # Directory where to save the results
            ...
            :normalize => true,                                        # Normalize the state during integration True=1, False=0
            ...
            :keplerian => false,                                        # Do not include drag passage: True=1, False=0, NOTE: Can't be used with aerobraking campaign, must be 'Orbits' in type_of_mission
            ...
            :solar_panel_control_rate => 0.1,                       # Integration of the equations of motion and lambda to define time switches and revaluation second time switch
            ...
            :ra_initial_a => 50.01e6 + 6.3781e6, # 56378e3,                    # Initial Apoapsis Radius for for-loop in m
            :ra_initial_b => 1e21,                                  # Final Apoapsis Radius for for-loop in m
            :ra_step => 5e21,                                       # Step Apoapsis Radius for for-loop in m
            :hp_initial_a => 120e3,   #200590.0,#                   # Initial Periapsis Altitude for for-loop in m
            :hp_initial_b => 9990000.0,                             # Final Periapsis Altitude for for-loop in m
            :hp_step => 1e20,                                       # Step Periapsis Radius for for-loop in m
            :v_initial_a => 8300.0,                                 # Initial Velocity (m/s) for for-loop if initial conditions are in v and gamma
            :v_initial_b => 40000.0,                                # Final Velocity (m/s) for for-loop if initial conditions are in v and gamma
            :v_step => 1e15,                                        # Step Velocity (m/s) for for-loop if initial conditions are in v and gamma
            :γ_initial_a => 3.0,                                    # Initial Gamma (deg) for for-loop if initial conditions are in v and gamma
            :γ_initial_b => 20.0,                                   # Final Gamma (deg) for for-loop if initial conditions are in v and gamma
            :γ_step => 100.0,                                       # Step Gamma (deg) for for-loop if initial conditions are in v and gamma
            ...
            # Final Conditions
            :final_apoapsis => 9000e3, # 45378.0e3, # 4905.974818462152e3                  # Final apoapsis radius if aerobraking campaign
            ...
            :ra_fin_orbit => 25700e3,                               # Target final apoapsis for the orbit, m
