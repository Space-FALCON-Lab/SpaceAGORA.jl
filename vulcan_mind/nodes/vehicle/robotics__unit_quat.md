---
id: vehicle.robotics__unit_quat
label: _unit_quat
kind: function
source:
...
Converts to `SVector{4,Float64}`, takes the norm, and divides when the norm is finite and above machine epsilon; otherwise returns the identity constant. `@inline` so it vanishes into callers on the FK hot path.
...
- [[grp.src_dynamics_multibody_cloth|dynamics/multibody_cloth/]] · `members_out` → `callers` · call · `src/dynamics/multibody_cloth/cloth_multibody.jl:166-166`
...
## Limitations
Falling back to identity hides a zero or NaN quaternion instead of raising, which can mask an upstream bug as an unrotated link.

## Provenance
Mapped from `src/vehicle/robotics/robotics.jl` line 75.
