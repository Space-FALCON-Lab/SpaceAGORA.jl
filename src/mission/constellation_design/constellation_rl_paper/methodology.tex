...
The \texttt{SatelliteSeedingEnv} implements the constellation RL environment following the OpenAI Gym interface. Key features include:
...
The training entry point \texttt{run\_constellation\_rl\_training\_campaign.jl} parses a YAML configuration, generates scenarios, and executes training in parallel or sequential mode.
...
This ensures that the constellation RL search space bounds match those used by stochastic greedy seeding for fair comparison.
