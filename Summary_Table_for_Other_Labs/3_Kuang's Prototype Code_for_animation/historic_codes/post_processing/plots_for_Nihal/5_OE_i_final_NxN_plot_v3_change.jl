# NxN grid of final inclination vs number of helper satellites.
# Rows = altitudes, Columns = inclinations.
# Each subplot shows i(end) for every helper count in filter_helpers.
...
main_ylabel          = L"\Delta i,\ \mathrm{deg}"
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        local res = try
            plot_OE(h, i_deg; element=:i, show_variance=false,
        ...
        if res === nothing
            ...
            # Final inclination for each helper count — sort ascending
            N_helpers_vec = res.N_vals .- 1
            if h == 1000; N_helpers_vec = [n == 249 ? 250 : n for n in N_helpers_vec]; end
            i_final_deg   = [v[end] - v[1] for v in res.all_vals]  # Δi in degrees (plot_OE already converts)
            ord           = sortperm(N_helpers_vec)
            N_helpers_vec = N_helpers_vec[ord]
            i_final_deg   = i_final_deg[ord]
            # Snap floating-point noise to zero; use ±20 deg range for zero data
            _is_zero = maximum(abs.(i_final_deg)) < 1e-6
            if _is_zero; fill!(i_final_deg, 0.0); end
            xs            = N_helpers_vec             # actual values → proper spacing

            _imin = minimum(i_final_deg); _imax = maximum(i_final_deg)
            _ictr  = (_imin + _imax) / 2
            _ihalf = max((_imax - _imin) / 2 * 1.1, abs(_ictr) * 0.5, 1e-9)
            ytk_vals, ytk_labs = _is_zero ? nice_ticks(-0.0020, 0.0020; n=5) :
                                            nice_ticks(_ictr - _ihalf, _ictr + _ihalf; n=5)
...
annotate!(legend_sp, 0.45, 0.5,
          text(L"\Delta i", :black, :left, legend_fontsize))
...
mkpath(output_dir)
out_path     = joinpath(output_dir, "OE_i_final_NxN_plot.png")
out_path_pdf = joinpath(output_dir, "OE_i_final_NxN_plot.pdf")
