...
function propulsion_ic_calcs(m, args, initial_state)
    ...
    if length(config.solution.performance.mass) == 0
        ...
        results = ones(config.solution.simulation.solution_states)
        ...
        results[98] = (config.solution.physical_properties.α_control[index_a] + (config.solution.physical_properties.α_control[index_b] - config.solution.physical_properties.α_control[index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[99] = (config.solution.physical_properties.inertia_tensor[1][index_a] + (config.solution.physical_properties.inertia_tensor[1][index_b] - config.solution.physical_properties.inertia_tensor[1][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[100] = (config.solution.physical_properties.inertia_tensor[2][index_a] + (config.solution.physical_properties.inertia_tensor[2][index_b] - config.solution.physical_properties.inertia_tensor[2][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[101] = (config.solution.physical_properties.inertia_tensor[3][index_a] + (config.solution.physical_properties.inertia_tensor[3][index_b] - config.solution.physical_properties.inertia_tensor[3][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[102] = (config.solution.physical_properties.inertia_tensor[4][index_a] + (config.solution.physical_properties.inertia_tensor[4][index_b] - config.solution.physical_properties.inertia_tensor[4][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[103] = (config.solution.physical_properties.inertia_tensor[5][index_a] + (config.solution.physical_properties.inertia_tensor[5][index_b] - config.solution.physical_properties.inertia_tensor[5][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[104] = (config.solution.physical_properties.inertia_tensor[6][index_a] + (config.solution.physical_properties.inertia_tensor[6][index_b] - config.solution.physical_properties.inertia_tensor[6][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[105] = (config.solution.physical_properties.inertia_tensor[7][index_a] + (config.solution.physical_properties.inertia_tensor[7][index_b] - config.solution.physical_properties.inertia_tensor[7][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[106] = (config.solution.physical_properties.inertia_tensor[8][index_a] + (config.solution.physical_properties.inertia_tensor[8][index_b] - config.solution.physical_properties.inertia_tensor[8][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[107] = (config.solution.physical_properties.inertia_tensor[9][index_a] + (config.solution.physical_properties.inertia_tensor[9][index_b] - config.solution.physical_properties.inertia_tensor[9][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[108] = (config.solution.physical_properties.τ_rw[1][index_a] + (config.solution.physical_properties.τ_rw[1][index_b] - config.solution.physical_properties.τ_rw[1][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[109] = (config.solution.physical_properties.τ_rw[2][index_a] + (config.solution.physical_properties.τ_rw[2][index_b] - config.solution.physical_properties.τ_rw[2][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[110] = (config.solution.physical_properties.τ_rw[3][index_a] + (config.solution.physical_properties.τ_rw[3][index_b] - config.solution.physical_properties.τ_rw[3][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))

        n_bodies = length(config.model.body.links)
        for i in 1:n_bodies
            results[110 + i] = (config.solution.physical_properties.α[i][index_a] + (config.solution.physical_properties.α[i][index_b] - config.solution.physical_properties.α[i][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
            results[110 + n_bodies + i] = (config.solution.physical_properties.β[i][index_a] + (config.solution.physical_properties.β[i][index_b] - config.solution.physical_properties.β[i][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
            results[110 + 2*n_bodies + i] = (config.solution.performance.heat_rate[i][index_a] + (config.solution.performance.heat_rate[i][index_b] - config.solution.performance.heat_rate[i][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
            results[110 + 3*n_bodies + i] = (config.solution.performance.heat_load[i][index_a] + (config.solution.performance.heat_load[i][index_b] - config.solution.performance.heat_load[i][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        end
        ...
        results[85] = (config.solution.forces.τ_ii[1][index_a] + (config.solution.forces.τ_ii[1][index_b] - config.solution.forces.τ_ii[1][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[86] = (config.solution.forces.τ_ii[2][index_a] + (config.solution.forces.τ_ii[2][index_b] - config.solution.forces.τ_ii[2][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[87] = (config.solution.forces.τ_ii[3][index_a] + (config.solution.forces.τ_ii[3][index_b] - config.solution.forces.τ_ii[3][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        ...
        results[91] = (config.solution.orientation.quaternion[1][index_a] + (config.solution.orientation.quaternion[1][index_b] - config.solution.orientation.quaternion[1][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[92] = (config.solution.orientation.quaternion[2][index_a] + (config.solution.orientation.quaternion[2][index_b] - config.solution.orientation.quaternion[2][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[93] = (config.solution.orientation.quaternion[3][index_a] + (config.solution.orientation.quaternion[3][index_b] - config.solution.orientation.quaternion[3][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[94] = (config.solution.orientation.quaternion[4][index_a] + (config.solution.orientation.quaternion[4][index_b] - config.solution.orientation.quaternion[4][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[95] = (config.solution.orientation.ω[1][index_a] + (config.solution.orientation.ω[1][index_b] - config.solution.orientation.ω[1][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[96] = (config.solution.orientation.ω[2][index_a] + (config.solution.orientation.ω[2][index_b] - config.solution.orientation.ω[2][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        results[97] = (config.solution.orientation.ω[3][index_a] + (config.solution.orientation.ω[3][index_b] - config.solution.orientation.ω[3][index_a]) / (dist_b + abs(dist_a)) * abs(dist_a))
        ...
        deleteat!(config.solution.orientation.quaternion[1], range(start=index_b+1,step=1,stop=length(config.solution.orientation.quaternion[1])))   
        deleteat!(config.solution.orientation.quaternion[2], range(start=index_b+1,step=1,stop=length(config.solution.orientation.quaternion[2])))   
        deleteat!(config.solution.orientation.quaternion[3], range(start=index_b+1,step=1,stop=length(config.solution.orientation.quaternion[3])))   
        deleteat!(config.solution.orientation.quaternion[4], range(start=index_b+1,step=1,stop=length(config.solution.orientation.quaternion[4])))
        deleteat!(config.solution.orientation.ω[1], range(start=index_b+1,step=1,stop=length(config.solution.orientation.ω[1])))   
        deleteat!(config.solution.orientation.ω[2], range(start=index_b+1,step=1,stop=length(config.solution.orientation.ω[2])))   
        deleteat!(config.solution.orientation.ω[3], range(start=index_b+1,step=1,stop=length(config.solution.orientation.ω[3])))
        ...
        deleteat!(config.solution.physical_properties.α_control, range(start=index_b+1,step=1,stop=length(config.solution.physical_properties.α_control)))
        
        for i in 1:n_bodies
            deleteat!(config.solution.physical_properties.α[i], range(start=index_b+1,step=1,stop=length(config.solution.physical_properties.α[i])))
            deleteat!(config.solution.physical_properties.β[i], range(start=index_b+1,step=1,stop=length(config.solution.physical_properties.β[i])))
        end
        for i in 1:config.model.body.n_reaction_wheels
            deleteat!(config.solution.physical_properties.rw_h[i], range(start=index_b+1,step=1,stop=length(config.solution.physical_properties.rw_h[i])))
            deleteat!(config.solution.physical_properties.rw_τ[i], range(start=index_b+1,step=1,stop=length(config.solution.physical_properties.rw_τ[i])))
        end
