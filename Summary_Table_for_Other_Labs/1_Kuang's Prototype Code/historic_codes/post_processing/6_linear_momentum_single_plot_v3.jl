# Single plot — Linear Momentum Fractional Drift ΔP/P₀
...
filter_N         = 100[50, 100, 150, 200]  # one or more helper counts, e.g. [50, 100, 150]  (nothing = all)
...
main_xlabel         = "t (hr)";  main_ylabel = "ΔP/P₀"
ylabel_gap          = 0.5;  xlabel_gap = 0.5
# ─────────────────────────────────────────────────────────────────────────────

# Build the list of filename fragments that every accepted CSV must contain
# filter_N is handled separately: accept a file if its N matches ANY value in the list
_N_frags = filter_N === nothing ? nothing :
           [@sprintf("_N%d_", n+1) for n in (filter_N isa AbstractArray ? filter_N : [filter_N])]
csv_filters = String[]
...
csv_files = isdir(data_dir) ?
    filter(f -> endswith(f,".csv") &&
               all(fr->occursin(fr,basename(f)), csv_filters) &&
               (_N_frags === nothing || any(fr->occursin(fr,basename(f)), _N_frags)),
           readdir(data_dir; join=true)) : String[]

isempty(csv_files) && error("No matching CSVs found in: $data_dir  (filters: $csv_filters)")

# Load all CSVs and group by N value (so we get exactly one grey line per N)
_curves_by_N = Dict{Int, Vector{Vector{Float64}}}()
t_common = nothing
for csv_path in csv_files
    ...
    c     = [interp1(sol.t, ΔP, t) for t in t_uni]
    N_val = p[:N]
    haskey(_curves_by_N, N_val) || (_curves_by_N[N_val] = Vector{Float64}[])
    push!(_curves_by_N[N_val], c)
    isnothing(t_common) && (global t_common = t_uni)
end

isempty(_curves_by_N) && error("Failed to load any CSVs.")

# One grey line per N = mean across all runs for that N
all_ΔP      = Vector{Vector{Float64}}()
all_Nlabels = String[]
for N_val in sort(collect(keys(_curves_by_N)))
    curves = _curves_by_N[N_val]
    mean_c = length(curves) == 1 ? curves[1] : vec(mean(hcat(curves...), dims=2))
    push!(all_ΔP, mean_c)
    push!(all_Nlabels, "N=$N_val")
end

t_hr    = t_common ./ 3600.0
mat     = hcat(all_ΔP...)
mean_ΔP = vec(mean(mat, dims=2))
std_ΔP  = vec(std(mat,  dims=2))

_sub_parts = [@sprintf("h=%dkm, i=%.1f°", h, i_deg)]
filter_N         !== nothing && push!(_sub_parts, "N=$(join(filter_N isa AbstractArray ? filter_N.+1 : [filter_N+1], ","))")
...
fig = plot(legend=:outertopright, grid=true, tick_direction=:out,
           xlabel="t (hr)", ylabel="ΔP/P₀",
           title="Linear Momentum Fractional Drift",
           titlefontsize=title_fontsize,
           top_margin=title_top_margin_mm*Plots.mm)
for (c, lbl) in zip(all_ΔP, all_Nlabels)
    plot!(fig, t_hr, c, color=:grey, alpha=0.5, lw=1, label=lbl)
end
plot!(fig, t_hr, mean_ΔP .+ std_ΔP,
      fillrange=mean_ΔP .- std_ΔP,
      fillalpha=0.2, fillcolor=:blue, linealpha=0, label="±1σ")
plot!(fig, t_hr, mean_ΔP, color=:blue, lw=2, label="Mean")
_yl = ylims(fig)
...
mkpath(output_dir)
out_path = joinpath(output_dir, @sprintf("lm_fractional_h%dkm_i%.1fdeg.png", h, i_deg))
