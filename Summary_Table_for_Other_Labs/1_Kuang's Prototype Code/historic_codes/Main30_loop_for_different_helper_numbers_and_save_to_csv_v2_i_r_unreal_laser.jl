...
helper_counts = 100 #vcat(150:50:300) #vcat(50:50:100) # vcat(50:50:300) #vcat(150:50:300) #vcat(1, 50:50:300) # vcat(50:50:200)
target_altitudes_km = 950 #850 # [950.0, 1000.0, 1050.0, 1150.0] # [850.0, 950.0, 1000.0, 1050.0, 1150.0] # [800]
target_inclinations_deg = 1.0 #[0.0, 0.5, 1.0] #[0.0, 0.5, 1.0, 1.5]
B_laser    = 2000.0   # cavity B parameter
Pin_laser  = 1e5      # cavity input power [W]
min_range  = 0.0      # min range limit for laser forces [m]
max_range  = 3000e3   # max range limit for laser forces [m]
...
for alt_km in target_altitudes_km
    for inc_deg in target_inclinations_deg
        case_csv_dir = normpath(joinpath(@__DIR__, "output", "CSV", @sprintf("target_h%.0fkm_i%.1fdeg", alt_km, inc_deg)))
        mkpath(case_csv_dir)
        mkpath(normpath(joinpath(@__DIR__, "output", "images")))
        ...
        for helper_num in helper_counts
            ...
            for j in 1:helper_num # Construct cav for helper satellites (1 to helper_num) linking to the target satellite (index N)
                cav[(j, helper_num + 1)] = Dict(:B => B_laser, :Pin => Pin_laser)
            end
