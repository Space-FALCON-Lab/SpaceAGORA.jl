...
function run_open_cavity_multi(oe_list::AbstractVector;
                               ...
                               dt_max_s::Real=Inf,
                               output_interval_s::Real=10.0)

    dt_max_s > 0 && !isnan(dt_max_s) || throw(ArgumentError("dt_max_s must be positive"))
    isfinite(output_interval_s) && output_interval_s > 0 || throw(ArgumentError("output interval must be finite and positive"))
    # 1. Obtain initial state u0 from initial OE of satellites
    ...
    p = Dict(:mu=>MU, :c=>C, :N=>N, :masses=>mvec, :Pmatrix=>Pm_use,
             ...
             :output_interval_s=>Float64(output_interval_s))
    ...
    recorder = record_events ? make_prototype_recorder(u0, p) : nothing
    event_callbacks = record_events ? recording_callbacks(recorder) : ()
    ...
    if stop_on_dv && 1 <= dv_target_sat <= N # augmented if dv stop requested on a valid index
        ...
        cb     = make_dv_stop_callback_cont(p_aug; sat=dv_target_sat,
                                            ...
                                            dirECI=dv_target_dirECI)
        sol = solve(prob, Vern9(); reltol=1e-12, abstol=1e-12,
            callback=CallbackSet(cb, event_callbacks...), saveat=output_interval_s, save_end=true, dtmax=dt_max_s)
    ...
    elseif stop_on_r && 1 <= r_target_sat <= N # augmented if dv stop requested on a valid index
        ...
        prob = ODEProblem(nbody_photon!, u0, (0.0, T), p)
        sol  = solve(prob, Vern9(); reltol=1e-12, abstol=1e-12,
            callback=CallbackSet(event_callbacks...), saveat=output_interval_s, save_end=true, dtmax=dt_max_s)
    end

    if record_events
        finish_recording!(recorder, sol.u[end], sol.t[end])
        p[:feather_recorder] = recorder
    end
