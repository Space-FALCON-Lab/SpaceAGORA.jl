...
function _with_scene_flag(args::SM.SimulationConfiguration, flag::Bool)
...
@testset "SceneVisualization" begin
    ...
    @testset "link_pose save field is opt-in" begin
        ...
        @test names_on[1:end-1] == names_off
        ...
        single_args = SM.SimulationConfiguration(
            file_paths=on.file_paths,
            simulation_settings=on.simulation_settings,
            mission_configuration=on.mission_configuration,
            environment_model=on.environment_model,
            dynamics_model=SM.DynamicsModel([single], on.dynamics_model.dynamic_effectors),
            guidance_model=on.guidance_model,
            navigation_model=on.navigation_model,
            control_model=on.control_model,
            initial_time=on.initial_time,
            integration_tolerances=on.integration_tolerances,
            solver_config=on.solver_config
        )
    ...
    @testset "end-to-end: flagged run writes sidecar and link_pose columns" begin
        ...
        dir_off = mktempdir()
        run_simulation(_short_config(results_directory=dir_off, flag=false))
