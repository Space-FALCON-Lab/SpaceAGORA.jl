# NxN grid of Linear Momentum Fractional Drift subplots.
# Rows = altitudes, Columns = inclinations.
# Each subplot calls plot_linear_momentum(h, i) for data; blank with "No Data" if unavailable.
...
margin_right  =  0.5
margin_top    =  0
tick_fontsize        = 7   # x and y tick number size
subtitle_fontsize    = 7   # per-subplot title font size
maintitle_fontsize   = 9   # main figure title font size
main_label_fontsize  = 9  # shared x/y axis label font size
main_xlabel          = "t (hr)"           # shared x-axis label
main_ylabel          = "ΔP/P(0) (×10⁻³)" # shared y-axis label
row_label_fontsize   = 7   # font size for h=Xkm row labels on first-column subplots
col_label_fontsize   = 7   # font size for i=X.X° col labels on bottom-row subplots
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        local sp
        local res = try
            plot_linear_momentum(h, i_deg; show_variance=true)
        catch
            nothing
        end
        ...
        if res === nothing
            ...
            sp = plot(legend=false, grid=false, framestyle=:box,
                      title=@sprintf("h=%dkm i=%.1f°", h, i_deg), titlefontsize=subtitle_fontsize,
...
#                   [ corner  | x-strip  ]
l = @layout [a{0.06w} grid(n_rows, n_cols) ; b{0.001w} c{0.001h}] # change gap arounf main xlabel and main ylabel here !!!!!

fig = plot(yl_sp, subplots..., corner_sp, xl_sp,
           layout=l,
           size=(200*n_cols + 70, 160*n_rows + 50),
           plot_title="Linear Momentum Fractional Drift  ΔP/P(0)",
...
mkpath(output_dir)
out_path = joinpath(output_dir, "linear_momentum_NxN_plot.png")
