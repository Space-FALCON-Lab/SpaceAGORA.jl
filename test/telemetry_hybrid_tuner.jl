...
function _write_report(
    ...
    open(path, "w") do io
        ...
        println(io)
        println(io, "```bash")
        println(io, "JULIA_NUM_THREADS=$(Threads.nthreads()) julia --project=.AGORA --startup-file=no scripts/verify_telemetry.jl --profile=full --manifest=$(best_manifest_path) --enforce=$(cfg.enforce ? "1" : "0")")
        println(io, "```")
    end
end
