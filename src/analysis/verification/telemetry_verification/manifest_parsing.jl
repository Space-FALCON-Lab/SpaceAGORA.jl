    ...
    for (idx, entry) in pairs(raw)
        ...
        elseif kind == "time_aligned_state"
            ...
            push!(scenarios, TimeAlignedScenarioConfig(
                ...
                telemetry_vx_col=haskey(ctbl, "vx") ? String(ctbl["vx"]) : nothing,
                telemetry_vy_col=haskey(ctbl, "vy") ? String(ctbl["vy"]) : nothing,
                telemetry_vz_col=haskey(ctbl, "vz") ? String(ctbl["vz"]) : nothing,
