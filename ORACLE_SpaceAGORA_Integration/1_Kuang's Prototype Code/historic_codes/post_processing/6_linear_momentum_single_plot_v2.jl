...
main_xlabel         = "t (hr)";  main_ylabel = "ΔP/P₀  [×10⁻³]"
...
for csv_path in csv_files
    ...
    push!(all_P, [interp1(sol.t, Pmag, t) for t in t_uni])
    isnothing(t_common) && (global t_common = t_uni)
end

isempty(all_P) && error("Failed to load any CSVs.")

t_hr   = t_common ./ 3600.0
mat    = hcat(all_P...)
mean_P = vec(mean(mat, dims=2))
std_P  = vec(std(mat,  dims=2))
...
fig = plot(legend=false, grid=true, tick_direction=:out,
           xlabel="t (hr)", ylabel="‖P‖  [kg·m/s]",
           title="Linear Momentum",
           titlefontsize=title_fontsize,
           top_margin=title_top_margin_mm*Plots.mm)
for c in all_P
    plot!(fig, t_hr, c, color=:grey, alpha=0.4, lw=0.5, label="")
end
plot!(fig, t_hr, mean_P .+ std_P,
      fillrange=mean_P .- std_P,
      fillalpha=0.2, fillcolor=:blue, linealpha=0, label="")
plot!(fig, t_hr, mean_P, color=:blue, lw=1.5, label="")
_yl = ylims(fig)
