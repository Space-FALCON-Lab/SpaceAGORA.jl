...
function _orbit_count_from_flat_sol(flat_sol::_FlatSol, mu::Float64; target_id::Int=length(first(flat_sol.u)) ÷ 6)
    T_series = [begin
        r = SVector{3,Float64}(u[idx(target_id,1)], u[idx(target_id,2)], u[idx(target_id,3)])
        v = SVector{3,Float64}(u[idx(target_id,4)], u[idx(target_id,5)], u[idx(target_id,6)])
...
#
# The aggregate laser-pair key is (0, target_id): "laser ensemble → target".
# =============================================================================
