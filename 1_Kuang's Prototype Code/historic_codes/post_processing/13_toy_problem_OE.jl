...
# ── Four separate plots: Δa, Δe, Δi, ΔΩ vs orbits ───────────────────────────
label_fontsize = 14   # axis label font size
tick_fontsize  = 12   # tick font size

common = (legend=false, lw=2, xformatter=:plain, yformatter=:scientific,
...
p_a = plot(oc, Δa; ylabel=L"\Delta a, \; \mathrm{m}",   color=:blue,   common..., yformatter=:plain)
p_e = plot(oc, Δe; ylabel=L"\Delta e",                   color=:red,    common...)
p_i = plot(oc, Δi; ylabel=L"\Delta i, \; \mathrm{deg}",  color=:green,  common...)
p_Ω = plot(oc, ΔΩ; ylabel=L"\Delta \Omega, \; \mathrm{deg}", color=:purple, common..., yformatter=:plain)

for (sp, tag) in zip((p_a, p_e, p_i, p_Ω), ("a", "e", "i", "Omega"))
    display(sp)
    savefig(sp, joinpath(output_dir, "delta_OE_$(tag)_toy_problem_$(csv_tag).png"))
    savefig(sp, joinpath(output_dir, "delta_OE_$(tag)_toy_problem_$(csv_tag).pdf"))
    println("Saved: delta_OE_$(tag)_toy_problem_$(csv_tag).png")
end
