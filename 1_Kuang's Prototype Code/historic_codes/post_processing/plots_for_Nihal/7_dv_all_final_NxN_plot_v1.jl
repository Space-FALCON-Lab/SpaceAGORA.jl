# 5x3 combined Δv grid: rows = altitudes, columns = [Δv_R, Δv_T, Δv_N].
# Each subplot shows 3 curves (one per target inclination: 0°, 0.5°, 1°).
# Colour-matched dashed vertical lines mark the saturation number per inclination.
...
fig_width  = 220*n_cols + 80   # ~740
fig_height = 180*n_rows + 50   # ~950

# ── Build subplots (row-major: for each altitude, loop over components) ───────
subplots = []
    ...
    for (col_idx, comp) in enumerate(components)
        ...
        for i_deg in target_inclinations_deg
            ...
            if res === nothing
                ...
                dvs   = comp == :R ? [ts.R[end] for ts in res.dv_timeseries] :
                        comp == :T ? [ts.T[end] for ts in res.dv_timeseries] :
        ...
        for (k, (xs, dvs, N_sat_val)) in enumerate(zip(all_xs, all_dvs, all_Nsat))
            isempty(xs) && continue
            plot!(sp, xs, dvs;
...
# ── Legend entry positions (left edge of each line segment, in [0,1]) ─────────
...
output_dir = normpath(joinpath(@__DIR__, "..", "..", "output",
                                "images_reconstructed_from_csv", "delta_v"))
mkpath(output_dir)
out_path     = joinpath(output_dir, "dv_all_final_NxN_plot.png")
out_path_pdf = joinpath(output_dir, "dv_all_final_NxN_plot.pdf")
