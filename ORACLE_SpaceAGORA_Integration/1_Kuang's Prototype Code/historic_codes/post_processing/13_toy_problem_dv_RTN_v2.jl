...
plt = plot(oc, dv_R;
           ...
           size=fig_size)
plot!(plt, oc, dv_T; label=L"\Delta v_T", color=:blue,  lw=2)
plot!(plt, oc, dv_N; label=L"\Delta v_N", color=:green, lw=2)
...
savefig(plt, joinpath(output_dir, "dv_RTN_toy_problem_$(csv_tag).pdf"))
