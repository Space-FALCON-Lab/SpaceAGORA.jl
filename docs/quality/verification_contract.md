...
2. `coverage-quality-gate` (`test/contracts/ci_coverage_quality_gate.jl`)
3. `p1-findings-gate` (`test/contracts/ci_p1_findings_gate.jl`)
4. `ai-review-artifact-gate` (`test/contracts/ci_ai_review_artifact_gate.jl`)
5. `architecture-contract-gate` (`test/contracts/ci_architecture_contract_gate.jl`)
6. `typed-config-equivalence-gate` (`test/contracts/ci_typed_config_equivalence_gate.jl`)
7. `benchmark-wrapper-parity-gate` (`test/contracts/ci_benchmark_wrapper_parity_gate.jl`)
8. `naming-contract-gate` (`test/contracts/ci_naming_contract_gate.jl`)
9. `no-legacy-ownership-gate` (`test/contracts/ci_no_legacy_ownership_gate.jl`)
10. `no-artifact-files-gate` (`test/contracts/ci_no_artifact_files_gate.jl`)
11. `canonical-path-contract-gate` (`test/contracts/ci_canonical_path_contract_gate.jl`)
12. `rotational-ownership-gate` (`test/contracts/ci_rotational_ownership_gate.jl`)

Additional required smoke checks for this migration track:
1. `test/contracts/ci_clean_depot_smoke.jl`
2. `test/contracts/ci_threaded_smoke.jl`
3. `test/contracts/ci_examples_regression.jl`
...
## Coverage Policy
Coverage enforcement is implemented by `test/contracts/ci_coverage_quality_gate.jl`.
...
Enforced by `test/contracts/ci_architecture_contract_gate.jl`.

## P1 Findings Policy
Enforced by `test/contracts/ci_p1_findings_gate.jl`.
...
## AI Artifact Policy
Enforced by `test/contracts/ci_ai_review_artifact_gate.jl`.
