...
function asim_ctrl_targeting(t_switch, param, time_0, in_cond; cnf=nothing)
    ...
    function f_ctrl_rf!(y_dot, in_cond, param, t0)
        ...
        if t0 >= t_switch
            ...
            state = [T_p, ρ, S]
            index_ratio = [1,1]
            cnf_state.α = _control_solarpanels_heatrate(ip, m, args, index_ratio, state, t0 - cnf_state.t_switch_targeting, cnf_state.initial_position_closed_form, OE; cnf=cnf_state)
        end
