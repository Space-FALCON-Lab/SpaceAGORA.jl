...
## Position-verification drivers (48-hour arc)

Added after the original study: a set of drivers that extend
`run_position_accuracy_48hr.jl` into a physics-complete verification of the
48-hour PVT arc. All of them need the private telemetry synced first
(`scripts/dev/fetch_private_telemetry.sh`; see
`data/telemetry/PRIVATE_TELEMETRY.md`).

- `harness_48hr_common.jl` — shared module includes and the
  reference/scenario helpers (same provenance as the copies inside
  `run_position_accuracy_48hr.jl`).
- `gram_wiring.jl` — include-world wiring for the GRAM extension hooks plus
  the offline-surrogate root setup, so drag runs work without a native GRAM
  build.
- `run_physics_ladder.jl` — conservative-force ladder (harmonics baseline,
  +Sun/Moon, +SRP), each rung scored against flight telemetry.
- `build_wam_density_table.jl` — samples NOAA WAM-IPE assimilated density
  along the flight track (requires WamIPEDensity.jl) into a private-side
  density table.
- `run_drag_decay_calibration.jl` — drag rung: fixed-cd-scale sweep with the
  table density and the spacecraft geometry read from a private-side TOML
  (`cygnss_science_geometry.toml`, which also selects
  `bus_ram_face = "frontal"` — the Hart-consistent reference-area
  convention added to the harness).
- `decay_postprocess.py` — extracts secular SMA decay slopes with a common
  estimator recipe, cancels estimator bias against a drag-free zero
  reference, and solves for the decay-matched cd-scale (insensitive to
  initial-condition error, unlike a position-RMSE fit).

Quantitative results are tracked in the (private) campaign results log, not
in this README.
