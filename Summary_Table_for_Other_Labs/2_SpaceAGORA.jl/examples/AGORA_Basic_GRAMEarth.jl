...
# This example is the smallest GRAM-backed path:
# - SPICE-backed Earth model
# - GRAM atmosphere model
# - same spacecraft/config structure as the no-GRAM quickstart
...
# Compared with the no-GRAM quickstart, the main changes are:
# - `Earth("", SPICE_PATH)` instead of `make_no_gram_planet(:earth)`
# - `GRAMAtmosphereModel(planet_name="earth")` instead of `NoAtmosphereModel()`
