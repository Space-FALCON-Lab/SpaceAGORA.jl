...
@testset "CygnssSlewTelemetry" begin
    ...
    @testset "epoch conversion" begin
        @testset "the origin is the J2000 epoch, not midnight that day" begin
            @test SLEW_TIME_ORIGIN == DateTime(2000, 1, 1, 12, 0, 0)
            @test slew_epoch_utc(0.0) == DateTime(2000, 1, 1, 12, 0, 0)
            @test slew_epoch_utc(86_400.0) == DateTime(2000, 1, 2, 12, 0, 0)
        end

        @testset "the FM01 export's first sample is 2025-10-04T00:58:07.5" begin
            # The export's own first time stamp; the value is quoted in the
            # module docstring and in the scenario script, so pin it here.
            stamp = slew_epoch_utc(8.12811487477591e8)
            @test Date(stamp) == Date(2025, 10, 4)
            @test Time(stamp) == Time(0, 58, 7, 478)
            # The alternative reading, seconds from midnight, would land twelve
            # hours earlier on the previous day. The SGP4 orbit-plane test in
            # scripts/dev/viewer_demos/cygnss_slew_checks.jl rejects it.
            other = DateTime(2000, 1, 1, 0, 0, 0) + Millisecond(round(Int, 8.12811487477591e8 * 1000))
            @test stamp - other == Hour(12)
            @test Date(other) == Date(2025, 10, 3)
        end

        @testset "sub-second resolution survives" begin
            @test slew_epoch_utc(1.5) == DateTime(2000, 1, 1, 12, 0, 1, 500)
        end
    end
    ...
    @testset "loaders" begin
        if !isfile(CYGSL_ADCS) || !isfile(CYGSL_PV)
            ...
            @test Date(slew_epoch_utc(tel.t_abs[1])) == Date(2025, 10, 4)
        end
