...
function plot_OE(h::Real, i_deg::Real;
    ...
    elseif element == :i
        all_i, "i (rad)",     "Inclination"
    elseif element == :Ω
        all_Ω, "Ω (rad)",     "RAAN"
    elseif element == :ω
        all_ω, "ω or u (rad)", "ω (non-circ) / u (circ)"
    else  # :ν
        all_ν, "ν (rad)",     "True anomaly"
    end
    ...
    plt = plot(title="$elem_label — target satellite ($dir_tag)",
               xlabel="t (hr)", ylabel=ylabel_str,
               formatter=:scientific, legend=:outertopright)

    for (vals, N) in zip(all_vals, N_vals)
        plot!(plt, t_hr, vals, label="N=$N", color=:grey, alpha=0.5, lw=1)
    end

    if show_variance
        plot!(plt, t_hr, mean_vals .+ std_vals,
              fillrange=mean_vals .- std_vals,
              fillalpha=0.2, fillcolor=:blue, linealpha=0, label="±1σ")
    end
    plot!(plt, t_hr, mean_vals, label="Mean", color=:blue, lw=2)
