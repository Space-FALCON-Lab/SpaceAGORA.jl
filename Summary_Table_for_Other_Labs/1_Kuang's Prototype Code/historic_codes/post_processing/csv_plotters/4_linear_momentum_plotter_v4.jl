...
plt2 = scatter(title="Momentum Variation ($dir_tag)", xlabel="t (s)",
               ylabel="ΔP = P(t) - P(0) (kg·m/s)", formatter=:scientific, legend=:outertopright)
...
for csv_path in csv_files
    ...
    ΔP   = Pmag .- Pmag[1]
    N = p[:N]
    scatter!(plt1, sol.t, Pmag, label="N=$N", ms=2)
    scatter!(plt2, sol.t, ΔP,   label="N=$N", ms=2)
end
...
savefig(plt2, joinpath(output_dir, "linear_momentum_variation_$(dir_tag).png"))
