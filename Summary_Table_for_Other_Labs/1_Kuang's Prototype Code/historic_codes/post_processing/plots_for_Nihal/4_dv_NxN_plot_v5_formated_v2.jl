...
subtitle_fontsize    = 8    # per-subplot title font size
maintitle_fontsize   = 8    # main figure title font size
main_label_fontsize  = 8    # shared x/y axis label font size
legend_fontsize      = 8    # legend font size
main_xlabel          = L"N_{\mathrm{helpers}}"                            # shared x-axis label
 main_ylabel          = L"\Delta v\ \mathrm{(final)}\ [\mathrm{m/s}]"  # shared y-axis label
...
ytick_left_margin    = -3   # mm — negative pulls tick numbers closer to y-axis; tune if labels clip
    ...
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        if res === nothing
            ...
            sp_l = plot(legend=false, grid=true,
                        ...
                        left_margin=ytick_left_margin*Plots.mm, right_margin=gap_inner*Plots.mm,
                        top_margin=gap_inner*Plots.mm,           bottom_margin=gap_inner*Plots.mm)
            ...
            if N_sat_val !== nothing && N_sat_val <= res.N_helpers_vec[end]
                ...
                vline!(sp_l, [x_sat]; color=:red, linestyle=:dash, lw=1.5, label=latexstring("N_{\\mathrm{sat}}=$(N_sat_val)"))
            end
            ...
            plot!(sp_r, xs, dv_final_T;
                  label=L"\Delta v_T", color=:orange, linestyle=:dash,    lw=1.5, marker=:circle, ms=3)
            plot!(sp_r, xs, dv_final_N;
                  label=L"\Delta v_N", color=:green,  linestyle=:dashdot, lw=1.5, marker=:circle, ms=3)
        end
...
plot!(legend_sp, [0,0], [2,2]; label=L"\Delta v_R",           color=:blue,   linestyle=:solid,   lw=1.5)
plot!(legend_sp, [0,0], [2,2]; label=L"\Delta v_T",           color=:orange, linestyle=:dash,    lw=1.5)
plot!(legend_sp, [0,0], [2,2]; label=L"\Delta v_N",           color=:green,  linestyle=:dashdot, lw=1.5)
plot!(legend_sp, [0,0], [2,2]; label=L"N_{\mathrm{sat}}",     color=:red,    linestyle=:dash,    lw=1.5)
