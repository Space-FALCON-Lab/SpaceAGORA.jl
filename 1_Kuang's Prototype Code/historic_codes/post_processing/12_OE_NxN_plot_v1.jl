...
include(joinpath(@__DIR__, "csv_plotter_functions", "9_OE_function_v1.jl"))
...
element_meta = Dict(
    ...
    :i => (ylabel="i (rad)",    title="Inclination",              fname="OE_i_NxN_plot.png"),
    :Ω => (ylabel="Ω (rad)",    title="RAAN",                     fname="OE_Omega_NxN_plot.png"),
    :ω => (ylabel="ω/u (rad)",  title="ω (non-circ) / u (circ)", fname="OE_omega_NxN_plot.png"),
    :ν => (ylabel="ν (rad)",    title="True anomaly",             fname="OE_nu_NxN_plot.png"),
)
    ...
    for (row_idx, h) in enumerate(target_altitudes_km)
        for (col_idx, i_deg) in enumerate(target_inclinations_deg)
            ...
            local elapsed = @elapsed begin
                res = try
                    plot_OE(h, i_deg; element=element, show_variance=true)
                catch e
            ...
            if res === nothing
                ...
                for curve in res.all_vals
                    plot!(sp, t_hr, curve, color=:grey, alpha=0.4, lw=0.5, label="")
                end
                # ±1σ band
                plot!(sp, t_hr, res.mean_vals .+ res.std_vals,
                      fillrange=res.mean_vals .- res.std_vals,
                      fillalpha=0.2, fillcolor=:blue, linealpha=0, label="")
                # mean line
                plot!(sp, t_hr, res.mean_vals, color=:blue, lw=1.2, label="")
            end
