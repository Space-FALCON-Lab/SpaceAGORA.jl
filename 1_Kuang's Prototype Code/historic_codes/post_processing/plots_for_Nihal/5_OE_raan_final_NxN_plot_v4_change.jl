...
for (row_idx, h) in enumerate(target_altitudes_km)
    ...
    all_rfinals  = Vector{Vector{Float64}}()
    ...
    for i_deg in target_inclinations_deg
        ...
        if res === nothing
            ...
            push!(all_rfinals, raan_final_deg[ord])
        end
    end

    all_vals_flat = vcat(all_rfinals...)
    ...
    _is_all_zero = maximum(abs.(all_vals_flat)) < 1e-6
    _rmin = minimum(all_vals_flat); _rmax = maximum(all_vals_flat)
    _rctr  = (_rmin + _rmax) / 2
    _rhalf = max((_rmax - _rmin) / 2 * 1.1, abs(_rctr) * 0.5, 1e-9)
    ytk_vals, ytk_labs = _is_all_zero ? nice_ticks(-20.0, 20.0; n=5) :
    ...
    for (k, (xs, yf)) in enumerate(zip(all_xs, all_rfinals))
        isempty(xs) && continue
        plot!(sp, xs, yf;
