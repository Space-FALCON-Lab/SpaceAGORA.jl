# Run the targeted supplemental inclination x argument-of-periapsis grid used
# to fill out poster regime-map slices.
#
# This is intentionally narrower than the default Monte Carlo:
# - nominal periapsis only
...
function _parse_supplement_args(args::Vector{String})
    ...
    output_dir = joinpath(AerobrakingPerturbationMC.REPO_ROOT, "output", "aerobraking_perturbation_mc_inclination_argp_supplement")
    resume = false
    resume_dir = ""
    ...
    while i <= length(args)
        ...
        elseif arg in ("-h", "--help")
            println("""
            Usage:
              julia --project=. benchmarks/studies/aerobraking_perturbation_mc/run_inclination_argp_supplement.jl [options]
