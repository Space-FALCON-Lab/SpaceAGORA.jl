...
args = Dict(# Misc Simulation
            ...
            :directory_results => "/workspaces/SpaceAGORA.jl_deprecated/journal_output/Titan_targ_pert_mc",       # Directory where to save the results
            ...
            :n_bodies => ["Saturn"], # ["Saturn"],                                # Add names of bodies you want to simulate the gravity of to a list. Keep list empty if not required to simulate extra body gravity.
            ...
            :v_step => 100000000.0,                                 # Step Velocity (m/s) for for-loop if initial conditions are in v and gamma

            :orientation_type => 0,
            :γ_initial_a => 12.5,                                   # Initial Gamma (deg) for for-loop if initial conditions are in v and gamma
            :γ_initial_b => 90.0,                                   # Final Gamma (deg) for for-loop if initial conditions are in v and gamma
            ...
            # Final Conditions
            :final_apoapsis => 10000e3 + 2.5755e6, # 4905.974818462152e3                  # Final apoapsis radius if aerobraking campaign
...
t = @elapsed begin
    ...
    # Run the simulation
    for i in collect(range(1,498,step=1))
...
#     for hp in [590e3, 610e3, 630e3] # [5000e3, 10000e3, 15000e3, 25000e3]
#         ra_initial_a = 10000e3
#         hp_initial_a = hp
#         inc          = args[:inclination]
#         RAAN         = args[:Ω]
#         AOP          = args[:ω]
...
#     # Run the simulation
#     for i in collect(range(1,72,step=1))
...
#     # Run the simulation
#     for vel in collect(range(2100,2200,step=25))
#         for gam in collect(range(-19.0,-6.0,step=0.25))
...
#         args[:α] = alph
#         for ra in collect(range(9000,14000,step=1000))
#             for hp in collect(range(600, 850,step=50))
#                 args[:ra_initial_a] = ra*1e3 + 2.575e6
