...
mutable struct Link
    ...
    net_force::SVector{3, Float64} # Net force acting on the link, to be updated at each simulation step
    net_torque::SVector{3, Float64} # Net torque acting on the link, to be updated at each simulation step
    function Link(;root=false,
                    ...
                    J_rw=zeros(3, gyro))#SMatrix{3,Int(gyro)}(1.0I))
        new(root, r, q, ṙ, ω, dims, ref_area, m, mass, inertia, a, b, gyro, rw, J_rw)
    end
end
...
mutable struct SpacecraftModel
    ...
    function SpacecraftModel(;joints=Joint[], links=Link[], roots=0.0, 
                        instant_actuation=true, 
                        prop_mass=Float64[])
        new(joints, links, roots, instant_actuation, prop_mass)
    end
end
...
function get_inertia_tensor(bodies::Vector{Link})
    """
...
function get_spacecraft_mass(model::SpacecraftModel, body::Link)
    """
    ...
    return total_mass + model.prop_mass[root_index] # Return the total mass
end

function get_spacecraft_mass(model::SpacecraftModel, bodies::Vector{Link}, root_index::Int)
    """
    ...
    total_mass = 0.0
    for body in bodies
        total_mass += body.m # Sum the mass of each body

    end
    return total_mass + model.prop_mass[root_index] # Return the total mass
end

function get_spacecraft_mass(model::SpacecraftModel)
    """
    ...
    for root in model.roots
        ...
        push!(total_mass + model.prop_mass[root_index]) # Return the total mass
    end
