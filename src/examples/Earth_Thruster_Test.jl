...
w = sqrt(6.0)
d = sqrt(66.0/7.0)
CSV.write("thruster_debug.csv", DataFrame(time=Float64[], on_time_request=Float64[], thrust_req=Float64[])) # Initialize the CSV file for thruster debugging
...
main_bus = config.Link(root=true, 
                        ...
                        m=750.0,
                        gyro=0,
                        attitude_control_rate=0.01, # seconds
...
args = Dict(# Misc Simulation
            ...
            :directory_results => "/workspaces/SpaceAGORA.jl/output/thruster_test",                # Directory where to save the results
            ...
            :eclipse => false,                                         # Eclipse true/false
            ...
            :save_rate => 50.0,                                      # Rate at which the data trajectory are saved
            ...
            :a_initial_b => 11000.0e3,                               # Final Semi-major axis for for-loop in m
            :a_step => 5e10,                                       # Step Semi-major axis for for-loop in m
            :e_initial_a => 0.01,                                   # Initial Eccentricity for for-loop in m
            :e_initial_b => 0.02,                                   # Final Eccentricity for for-loop in m
            ...
            :dt_max_orbit => 0.01,                                   # Maximum time step for orbit integration (outside atmosphere, i.e., step 1 and step 3), s
