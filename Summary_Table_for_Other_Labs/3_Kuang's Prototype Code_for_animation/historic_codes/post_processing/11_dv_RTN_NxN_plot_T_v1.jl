# NxN grid of cumulative Δv_T subplots.
# Rows = altitudes, Columns = inclinations.
# Each subplot calls plot_dv_RTN(h, i; component=:T) for data; blank with "No Data" if unavailable.
...
main_ylabel          = "Δv_T (m/s)" # shared y-axis label
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        local sp
        local res = try
            plot_dv_RTN(h, i_deg; component=:T, show_variance=true)
        catch e
            println("\n=== ERROR h=$h i=$i_deg ===")
            println(typeof(e), ": ", e)
...
fig = plot(yl_sp, subplots..., corner_sp, xl_sp,
           ...
           plot_title="Cumulative Δv_T — target satellite",
...
mkpath(output_dir)
out_path = joinpath(output_dir, "dv_T_NxN_plot.png")
