...
main_xlabel          = L"\mathrm{Number\ of\ orbits\ [target\ satellite]}"
...
element_meta = Dict(
    :a => (ylabel=L"a,\ \mathrm{m}",               title="Semi-major axis",          fname="OE_a_NxN_plot.png"),
    :e => (ylabel=L"e",                             title="Eccentricity",             fname="OE_e_NxN_plot.png"),
    :i => (ylabel=L"i,\ \mathrm{deg}",              title="Inclination",              fname="OE_i_NxN_plot.png"),
    :Ω => (ylabel=L"\Omega,\ \mathrm{deg}",         title="RAAN",                     fname="OE_Omega_NxN_plot.png"),
    :ω => (ylabel=L"\omega/u,\ \mathrm{deg}",       title="ω (non-circ) / u (circ)", fname="OE_omega_NxN_plot.png"),
    :ν => (ylabel=L"\nu,\ \mathrm{deg}",            title="True anomaly",             fname="OE_nu_NxN_plot.png"),
)
...
for element in elements_to_plot
    ...
    for (row_idx, h) in enumerate(target_altitudes_km)
        for (col_idx, i_deg) in enumerate(target_inclinations_deg)
            ...
            col_xlab  = is_bottom ? latexstring("i=$(i_deg)^\\circ")         : ""
