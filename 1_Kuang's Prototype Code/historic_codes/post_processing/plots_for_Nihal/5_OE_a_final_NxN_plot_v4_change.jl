...
for (row_idx, h) in enumerate(target_altitudes_km)
    ...
    # Collect data for all inclinations first (to compute shared y-range)
    all_xs      = Vector{Vector{Float64}}()
    all_yfinals = Vector{Vector{Float64}}()
    ...
    for i_deg in target_inclinations_deg
        ...
        if res === nothing
            push!(all_xs, Float64[])
            push!(all_yfinals, Float64[])
        else
            N_helpers_vec = res.N_vals .- 1
            if h == 1000; N_helpers_vec = [n == 249 ? 250 : n for n in N_helpers_vec]; end
            a_final = [v[end] - v[1] for v in res.all_vals]
            ord = sortperm(N_helpers_vec)
            push!(all_xs,      Float64.(N_helpers_vec[ord]))
            push!(all_yfinals, a_final[ord])
        end
    end

    # Shared y-axis range across all inclinations
    all_vals_flat = vcat(all_yfinals...)
    ...
    _amin = minimum(all_vals_flat); _amax = maximum(all_vals_flat)
    _actr  = (_amin + _amax) / 2
    _ahalf = max((_amax - _amin) / 2 * 1.1, abs(_actr) * 0.5, 1e-9)
    ytk_vals, ytk_labs = nice_ticks(_actr - _ahalf, _actr + _ahalf; n=5)

    # Use x-ticks from first non-empty inclination
    ...
    for (k, (xs, yf)) in enumerate(zip(all_xs, all_yfinals))
        isempty(xs) && continue
        plot!(sp, xs, yf;
