# NxN grid of cumulative Δv_R subplots.
# Rows = altitudes, Columns = inclinations.
# Each subplot calls plot_dv_RTN(h, i; component=:R) for data; blank with "No Data" if unavailable.

include(joinpath(@__DIR__, "csv_plotter_functions", "8_dv_RTN_function_v1.jl"))
...
main_ylabel          = "Δv_R (m/s)" # shared y-axis label
...
for (row_idx, h) in enumerate(target_altitudes_km)
    for (col_idx, i_deg) in enumerate(target_inclinations_deg)
        ...
        local elapsed = @elapsed begin
            res = try
                plot_dv_RTN(h, i_deg; component=:R, show_variance=true)
            catch e
                println("\n=== ERROR h=$h i=$i_deg ===")
                println(typeof(e), ": ", e)
        ...
        if res === nothing
            ...
            for dv_curve in res.all_dv
                plot!(sp, t_hr, dv_curve, color=:grey, alpha=0.4, lw=0.5, label="")
            end
            # ±1σ band
            plot!(sp, t_hr, res.mean_dv .+ res.std_dv,
                  fillrange=res.mean_dv .- res.std_dv,
                  fillalpha=0.2, fillcolor=:blue, linealpha=0, label="")
            # mean line
            plot!(sp, t_hr, res.mean_dv, color=:blue, lw=1.2, label="")
        end
...
fig = plot(yl_sp, subplots..., corner_sp, xl_sp,
           ...
           plot_title="Cumulative Δv_R — target satellite",
...
mkpath(output_dir)
out_path = joinpath(output_dir, "dv_R_NxN_plot.png")
