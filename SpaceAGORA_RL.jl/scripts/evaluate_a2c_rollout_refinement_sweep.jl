...
Usage:
  julia --project=SpaceAGORA_RL.jl \\
    SpaceAGORA_RL.jl/scripts/evaluate_a2c_rollout_refinement_sweep.jl

Evaluates rollout lengths 15, 16, and 17 using their best validation
checkpoints. Runs 100 shared perturbed-wind campaigns per policy with 16
workers, includes the same best PR-DRL checkpoint, and writes labeled plots and
...
function main(args=ARGS)
    ...
    isfile(definition_path) || error(
        "sweep definition does not exist; run run_a2c_rollout_refinement_sweep.jl first",
    )
