# Run a targeted periapsis-altitude supplement for the poster omega-vs-periapsis
# reference slice.
#
# This is intentionally narrow:
# - one representative apoapsis per body
# - inclination fixed at 93 deg
# - full omega grid
# - seven periapsis altitude levels spanning the current deep-to-shallow range
...
const SUPPLEMENT_PERIAPSIS_REGIMES = [:p01, :p02, :p03, :p04, :p05, :p06, :p07]

const SUPPLEMENT_PERIAPSIS_ALTITUDES_KM = Dict{Symbol, Vector{Float64}}(
    :mars => collect(range(110.0, 150.0; length=7)),
    :venus => collect(range(135.0, 180.0; length=7)),
    :earth => collect(range(120.0, 180.0; length=7)),
    :titan => collect(range(650.0, 900.0; length=7)),
)

function _periapsis_altitude_map_m()
    out = Dict{Tuple{Symbol, Symbol}, Float64}()
    for (planet, alts_km) in SUPPLEMENT_PERIAPSIS_ALTITUDES_KM
        for (regime, alt_km) in zip(SUPPLEMENT_PERIAPSIS_REGIMES, alts_km)
            out[(planet, regime)] = Float64(alt_km) * 1e3
        end
    end
    return out
end
...
function _parse_supplement_args(args::Vector{String})
    ...
    output_dir = joinpath(AerobrakingPerturbationMC.REPO_ROOT, "output", "aerobraking_perturbation_mc_periapsis_altitude_supplement")
    resume = false
    resume_dir = ""
    ...
    while i <= length(args)
        ...
        elseif arg in ("-h", "--help")
            println("""
            Usage:
              julia --project=. benchmarks/studies/aerobraking_perturbation_mc/run_periapsis_altitude_supplement.jl [options]
...
function main(args=ARGS)
    ...
    spec = AerobrakingPerturbationMC.StudySpec(
        ...
        periapsis_regimes=copy(SUPPLEMENT_PERIAPSIS_REGIMES),
        periapsis_altitudes_m=_periapsis_altitude_map_m(),
        ...
        inclinations_deg=[AerobrakingPerturbationMC.NOMINAL_INCLINATION_DEG],
