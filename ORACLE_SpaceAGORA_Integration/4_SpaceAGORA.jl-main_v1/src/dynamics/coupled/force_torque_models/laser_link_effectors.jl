...
function OpenCavityLaserLinkModel(;
    ...
    # Step 1: convert constructor inputs to the model's stored field types.
    ...
    # Step 2: validate the model and initialize its helper-state buffer.
...
function _validate_laser_link_model!(model::OpenCavityLaserLinkModel)::Nothing
    # Step 1: validate indices, helper selection, and numeric parameters.
    ...
    # Step 2: validate the selected scheduling policy.
    ...
    # Step 3: report successful validation.
    return nothing
end
...
@inline function _ensure_laser_link_state!(model::OpenCavityLaserLinkModel)::Nothing
    # Step 1: resize and reset the previous-range buffer when helper count changes.
...
@inline function solver_partition(::OpenCavityLaserLinkModel)
    # Step 1: classify laser thrust as an explicit, non-stiff contribution.
    return :explicit
end
...
@inline function _helper_slot(model::OpenCavityLaserLinkModel, helper_idx::Int)::Union{Nothing, Int}
    # Step 1: find the helper's position in the configured helper list.
    return findfirst(==(helper_idx), model.helper_indices)
end
...
function _in_range_flags!(
    ...
    # Step 1: ensure the model state and reusable flags buffer match the helpers.
    _ensure_laser_link_state!(model)
...
@inline function _rtn_basis(pos_t::SVector{3, Float64}, vel_t::SVector{3, Float64})
    # Step 1: compute the radial unit vector, with a fallback at zero radius.
    rnorm = norm(pos_t)
    rhat = rnorm > 0.0 ? pos_t / rnorm : SVector{3, Float64}(1.0, 0.0, 0.0)
    # Step 2: compute the orbit-normal unit vector, with a fallback at zero angular momentum.
    h = cross(pos_t, vel_t)
    hnorm = norm(h)
    nhat = hnorm > 0.0 ? h / hnorm : SVector{3, Float64}(0.0, 0.0, 1.0)
    # Step 3: compute the along-track unit vector and return the RTN basis.
...
function _activate_helper!(model::OpenCavityLaserLinkModel, helper_idx::Int)::Nothing
    # Step 1: count a new active link when the selected helper changes.
    if helper_idx > 0 && model.active_helper_idx != helper_idx
        model.link_activation_count += 1
    end
    # Step 2: store the active helper, with zero meaning inactive.
...
function update_laser_link_schedule!(
    ...
    # Step 1: validate the model and calculate current in-range helpers.
    _validate_laser_link_model!(model)
    in_range = _in_range_flags!(Bool[], model, pos)

    # Step 2: apply the configured scheduling policy.
...
@inline function laser_link_active_pair(model::OpenCavityLaserLinkModel)::Tuple{Int, Int}
    # Step 1: return the target and currently active helper indices.
    return (model.target_idx, model.active_helper_idx)
end
...
@inline function laser_link_force_magnitude(model::OpenCavityLaserLinkModel)::Float64
    # Step 1: calculate force from efficiency, reflectivity, magnification, power, and c.
    return model.eta * model.beta * model.magnification * model.power_w / SPEED_OF_LIGHT_MPS
end
...
function laser_link_pair_force(
    ...
    # Step 1: compute the helper-to-target displacement and range.
    rel = target_pos - helper_pos
    rho = norm(rel)
    (rho > 0.0 && rho <= model.range_m) || return SVector{3, Float64}(0.0, 0.0, 0.0)
    # Step 2: return the force vector when the helper is in range.
...
function accumulate_laser_link_forces!(
    ...
    # Step 1: stop when there is no valid active link or usable state.
    ...
    # Step 2: compute the active pair's force and apply equal-and-opposite totals.
...
function _update_matching_laser_models!(template::OpenCavityLaserLinkModel, integrator)::Nothing
    # Step 1: inspect every dynamic effector in the integrator.
    ...
    # Step 2: report completion after updating matching laser models.
    return nothing
end
        ...
        if dt > 0.0
            ...
            if helper_idx > 0
                ...
                if rho > 0.0 && rho <= model.range_m
                    ...
                    rhat, that, nhat = _rtn_basis(tgt_pos, tgt_vel)
                    accel = force / mass_kg
                    # Step 4: accumulate RTN delta-V and apply the equal-and-opposite kick.
...
function tracked_dv_at(tracker::LaserImpulseTracker, t::Float64)
    # Step 1: return zero when no callback history has been recorded.
    isempty(tracker.t_hist) && return (0.0, 0.0, 0.0)
    # Step 2: find the latest recorded time not after the query time.
    k = clamp(searchsortedlast(tracker.t_hist, t), 1, length(tracker.t_hist))
    # Step 3: return the cumulative RTN delta-V at that history entry.
