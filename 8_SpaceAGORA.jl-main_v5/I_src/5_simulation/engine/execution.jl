## 1. Stacking spacecraft's state block
# Purpose: Build a sparse Jacobian pattern allowing dependencies within each spacecraft's state block.
# Input: Initial ComponentVector containing all spacecraft states.
# Output: A sparse Float64 matrix with ones inside spacecraft blocks and zeros between them.
...
## 2. Building ODE Problem
# Purpose: Create the ODE problem matching the selected solver mode, without solving it.
# Input: Initial state, time span, parameters, callbacks, solver mode, and optional Jacobian pattern.
# Output: An ODEProblem, SplitODEProblem, or SecondOrderODEProblem ready for the solver.
...
# Purpose: Create an ODE problem using the currently active solver configuration.
# Input: Initial state, time span, parameters, callbacks, and optional Jacobian pattern.
# Output: The problem returned by the explicit-solver-mode overload.
...
## 3. Saving Results
# 3.1. save result to memory
# Purpose: Append saved samples from a gravity-backbone segment, skipping its first point.
# Input: Time/data accumulators, segment solution, requested save fields, and ODE parameters.
# Output: nothing; appends sample times and snapshots to the supplied accumulators.
...
# Purpose: Append a checkpoint segment's saved data and include its final point if missing.
# Input: Time/data accumulators, saved callback values, segment solution, save fields, and ODE parameters.
# Output: nothing; updates the accumulators with saved samples and any missing final snapshot.
...
# 3.2. save result to CSV and optionally to typed bundle
# Purpose: Write the appropriate saved simulation data when result output is enabled.
# Input: Configuration, solver mode, checkpoint flag, save fields, and normal/checkpoint/backbone saved data.
# Output: The CSV path, or nothing if results are disabled; writes files according to output settings.
...
# Wrapper around the _save_simulation_results_if_enabled! function that catches and logs errors.
...
## 4. Run simulation
# Purpose: Prepare and run the spacecraft simulation, optionally resuming checkpoints and saving results.
# Input: Simulation configuration and keywords for state isolation, returned data, save fields, callbacks, and solver cache.
# Output: nothing unless return_solution is true; then the last solution (possibly nothing) or a solution/metadata named tuple; may write files.
function run_simulation(
    ...
    # 4.1. Initialize simulation environment and policy context to preparefore any possible parallel computing request
    ...
    # 4.3. Reset parallel-policy bookkeeping
    try
    ...
    # 4.4. Set up the model and initial conditions
