...
function plot_oracle_laser_results(
    ...
    # ── Plot 1: Cumulative ΔV (R, T, N) vs time ────────────────────────────
    p1 = Plots.plot(
        tracker.t_hist, tracker.dv_R_hist;
        label="ΔvR", xlabel="time (s)", ylabel="cumulative Δv (m/s)",
        title="Laser Δv in RTN frame (target)", lw=2
    )
    Plots.plot!(p1, tracker.t_hist, tracker.dv_T_hist; label="ΔvT", lw=2)
    Plots.plot!(p1, tracker.t_hist, tracker.dv_N_hist; label="ΔvN", lw=2)
