...
filter_N         = 100   # helper count (nothing = all)
filter_B         = 2000   # e.g. 100.0
filter_Pin       = 1e5   # e.g. 1e4
filter_min_range = 0.0   # set both or neither
filter_max_range = 3000e3

# ── Layout Knobs ──────────────────────────────────────────────────────────────
fig_size            = (950, 800) #(800, 950)   # (width_px, height_px) — overall figure size
panel_heights       = [0.34, 0.33, 0.33]  # fractional height of each panel (must sum to 1)
title_fontsize      = 11
subtitle_fontsize   = 8
title_top_margin_mm = 5     # mm of space above axes of top panel
subtitle_drop_frac  = -0.08 # subtitle y-offset from top edge (0 = flush, 0.1 = 10% down)
...
for csv_path in csv_files
    ...
    push!(all_ΔP, [interp1(sol.t, ΔP, t) for t in t_uni])
    push!(all_ΔH, [interp1(sol.t, ΔH, t) for t in t_uni])
...
_draw_panel!(p1, t_hr, all_ΔP, mean_ΔP, std_ΔP; scale=1.0)
_yl = ylims(p1)
...
p2 = plot(legend=false, grid=true, tick_direction=:out,
          xlabel="", ylabel="ΔH/H₀  [×10⁻¹¹]",
          title="Angular Momentum Fractional Drift",
          titlefontsize=title_fontsize,
          top_margin=panel_top_margin_mm*Plots.mm)
_draw_panel!(p2, t_hr, all_ΔH, mean_ΔH, std_ΔH; scale=1e11)
...
p3 = plot(legend=false, grid=true, tick_direction=:out,
          ...
          titlefontsize=title_fontsize,
          top_margin=panel_top_margin_mm*Plots.mm)
_draw_panel!(p3, t_hr, all_ΔE, mean_ΔE, std_ΔE; scale=1e6)
