...
const START_APOAPSIS_ORBIT = 16
const START_APOAPSIS_TIME_S = 835980.353819746

planet = Mars("", SPICE_PATH)
smoke_mode = get(ENV, "SPACEAGORA_EXAMPLE_SMOKE", "0") == "1"
initial_time = InitialTime(year=2001, month=11, day=16, hour=2, minute=18, second=13.053653)
...
base_args = make_example_config(
    ...
    EI_km=160.0,
    results_directory=joinpath(REPO_ROOT, "output", "odyssey_start_apoapsis_16")
)
...
odyssey_plot_schedule = (
    maneuver_orbit_number=odyssey_schedule.maneuver_orbit_number .- 1,
