...
function eclipse_are_calc(r_sat::Vector{Float64}, r_sun::Vector{Float64}, A::Float64, rp::Float64)
    """
...
function srp(p, p_srp_unscaled::Float64, cR::Float64, A_sat::Float64, m::Float64, r_sat::Vector{Float64}, time_utc::String)
    """
    ...
    r_sat : Vector{Float64}
        Position vector of the satellite relative to the planet.
    time_utc : String
        UTC time of the simulation, used for SPICE calculations.
    ...
    r_sun, ltime = spkpos("SUN", str2et(time_utc), "J2000", "NONE", uppercase(p.name))
