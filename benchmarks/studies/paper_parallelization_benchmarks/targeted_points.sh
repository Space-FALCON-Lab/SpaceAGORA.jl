...
# against its best pinned static route, 11 repeats, 32 threads. Meant to run
# inside a release on the benchmark box (via scripts/remote/spaceagora-remote),
# one POINT per job, with the calibration store set by the launcher beforehand
# (cold = emptied; converged = the archived snapshot restored). Gates a full
# P1-P5 run: no point spending eleven hours to learn what one hour would.
set -euo pipefail
