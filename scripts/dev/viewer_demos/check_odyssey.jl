...
nrow(df) > 0 || throw(ArgumentError("saved Odyssey table is empty"))
length(lp) == 14 || throw(ArgumentError("expected two saved link poses (14 scalar columns), got $(length(lp))"))
...
println("panel alpha changes: n=", length(chg), " first t=", isempty(chg) ? NaN : df.time[chg[1]+1], " last t=", isempty(chg) ? NaN : df.time[chg[end]+1], " tEnd=", df.time[end])
