include("FeatherRecording.jl")
using .FeatherRecording
using TOML, SHA

module OracleFeatherCSV
include(joinpath(@__DIR__, "..", "..", "..", "1_Kuang's Prototype Code", "extract_feather_encounters.jl"))
end
...
function run_open_cavity_case_native(opts::OracleCase2Options)
    ...
    paths = scenario_paths(opts.output_dir, opts, _target_period_pre * opts.orbits;
        source="spaceagora", smoke=opts.smoke)
    results_dir = paths.feather
    ...
    r0  = SVector{3, Float64}(u0.sc[laser_model.target_idx].pos)
    v0  = SVector{3, Float64}(u0.sc[laser_model.target_idx].vel)
    ...
    state_adapter(state) = hcat([vcat(collect(satellite.pos), collect(satellite.vel)) for satellite in state.sc]...)
    function active_pairs(states, time)
        laser_model.eta * laser_model.beta * laser_model.magnification * laser_model.power_w > 0 ||
            return Tuple{Int,Int}[]
        helpers = laser_model.schedule === :external_schedule ? laser_model.active_helper_indices :
            (laser_model.active_helper_idx > 0 ? [laser_model.active_helper_idx] : Int[])
        return [minmax(laser_model.target_idx, helper) for helper in helpers
            if 0 < norm(states[1:3, helper]-states[1:3, laser_model.target_idx]) <= laser_model.range_m]
    end
    recorder = IntervalRecorder(state_adapter, active_pairs, u0, opts.laser_range_km*1e3)
    geometry_cb, observer_cb = recording_callbacks(recorder)
    ...
    # still captures output points independently at the configured interval.
    result = withenv("SPACEAGORA_SOLVER_SAVE_EVERYSTEP" => "false") do
        run_simulation(
            ...
            extra_callbacks=(geometry_cb, impulse_cb, scheduler_cb, observer_cb),
    ...
    finish_recording!(recorder, sol.u[end], sol.t[end])

    # --- Post-Analysis ---
    # Step 8: Read the simulation_results.feather saved by the solver
    native_path = joinpath(results_dir, "simulation_results.feather")
    feather_path = joinpath(results_dir, "trajectory.feather")
    feather_df = DataFrame(Arrow.Table(native_path))
    feather_df.time == output_times(sol.t[end], opts.output_interval_s) || error("Native output does not match the configured grid")
    Arrow.write(feather_path, feather_df; metadata=Dict(
        "source"=>"SpaceAGORA", "scenario"=>paths.scenario,
        "target_id"=>string(laser_model.target_idx),
        "maximum_range_m"=>string(opts.laser_range_km*1e3), "output_interval_s"=>string(opts.output_interval_s)))
    write_intervals(recorder, results_dir; source="SpaceAGORA", scenario=paths.scenario,
        laser_timing="accepted-step scheduler transitions and continuous range roots; nonzero selected force")
    manifest_path = joinpath(results_dir, "simulation_results.manifest.toml")
    manifest = TOML.parsefile(manifest_path)
    manifest["scenario"] = paths.scenario
    manifest["files"] = Dict(name => Dict("path"=>joinpath(results_dir, name*".feather"),
        "size_bytes"=>filesize(joinpath(results_dir, name*".feather")),
        "sha256"=>open(bytes2hex ∘ sha256, joinpath(results_dir, name*".feather")))
        for name in ("trajectory", "geometry_encounters", "laser_on"))
    ...
    orbit_counts   = _orbit_count_from_flat_sol(flat_sol, args.environment_model.planet.μ; target_id=laser_model.target_idx)
    orbits_elapsed = orbit_counts[end]

    # Step 10: Final orbital state from sol.u[end]
    final_state = sol.u[end].sc[laser_model.target_idx]
    ...
    if !opts.feather_only
        csv_result = OracleFeatherCSV.extract_feather_encounters(feather_path; output_dir=paths.csv)
        _write_csv!(joinpath(dirname(csv_result.extrema_path), "summary.csv"), [summary])
    end
...
`target_only = true` limits the per-satellite series to the target (sc[N]).
"""
