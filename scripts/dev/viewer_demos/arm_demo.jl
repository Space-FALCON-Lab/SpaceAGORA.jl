...
options = viewer_demo_options("arm_demo", 60.0)
outdir = options.output_dir
basecfg = make_example_config(planet=planet, spacecraft=sc, mission_time=options.duration_s,
...
args = SM.SimConfig._with_configuration(basecfg;
    mission_configuration=SM.MissionConfiguration(mission_type=basecfg.mission_configuration.mission_type, keplerian=true, number_of_orbits=1,
        mission_time=options.duration_s, orientation_sim=true, num_steps_to_save=1000, data_rate=0.1),
    control_model=SM.ControlModel(control_effectors=(eff,), control_rates=[0.1]))
