...
We evaluate the RL seeding method on debris clusters from the CAPOConstellation catalog. The experiments use:
...
Table~\ref{tab:quality} compares the RL seeding method with stochastic greedy baselines on individual clusters.
...
The RL method achieves comparable or better constellation quality than stochastic greedy with fewer satellites. FHSG achieves the best quality but requires significantly more computation.
...
The RL method is 10-100x faster than stochastic greedy and 100-1000x faster than FHSG after training. The training cost is amortized across all subsequent seeding runs.
