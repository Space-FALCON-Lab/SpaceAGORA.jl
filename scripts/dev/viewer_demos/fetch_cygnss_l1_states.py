...
TOKEN_PATH = Path(os.environ.get("EARTHDATA_TOKEN_FILE", str(Path.home() / ".edl_token")))
EXPECTED_SPACECRAFT = {1, 2, 3, 4, 5, 7, 8}
...
def dap4_dechunk(blob: bytes):
    ...
    while offset < len(blob):
        if len(blob)-offset < 4:
            raise RuntimeError("truncated DAP4 chunk header")
        flags = blob[offset]
        length = int.from_bytes(blob[offset + 1:offset + 4], "big")
        offset += 4
        if offset+length > len(blob):
            raise RuntimeError("truncated DAP4 chunk body")
        ...
        if flags & 0x01 and parts:
            ended = True
            break
    if not ended or offset != len(blob):
        raise RuntimeError("incomplete or trailing DAP4 chunks")
    if dmr is None:
...
def dap4_read(blob: bytes):
    ...
    if offset != len(data):
        raise RuntimeError("unexpected trailing DAP4 data")
    return out, dmr
...
def _bearer_header() -> dict:
    ...
    if not TOKEN_PATH.is_file():
        raise RuntimeError("Earthdata token unavailable: set EARTHDATA_TOKEN_FILE to a local token file; never put tokens in commands or source")
...
def earthdata_url(url):
    parts = urllib.parse.urlparse(url)
    host = parts.hostname or ""
    return parts.scheme == "https" and (host == "earthdata.nasa.gov" or host.endswith(".earthdata.nasa.gov")) and not parts.username


class EarthdataRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        if req.has_header("Authorization") and not earthdata_url(newurl):
            raise RuntimeError("refusing to forward Earthdata credentials outside NASA Earthdata HTTPS")
        return super().redirect_request(req, fp, code, msg, headers, newurl)


def _get(url: str, headers: dict | None = None, timeout: int = 300) -> bytes:
    if headers and "Authorization" in headers and not earthdata_url(url):
        raise RuntimeError("refusing credentials for non-Earthdata URL")
    request = urllib.request.Request(url, headers=headers or {})
    with urllib.request.build_opener(EarthdataRedirect()).open(request, timeout=timeout) as response:
...
def validate_inventory(granules):
    expected = {(fm, f"202506{day:02d}") for fm in EXPECTED_SPACECRAFT for day in range(6,10)}
    keys = []
    for g in granules:
        m = re.match(r"cyg(\d{2})\.ddmi\.s(\d{8})-", g["title"])
        if not m or Path(g["title"]).name != g["title"] or not earthdata_url(g["opendap"]):
            raise RuntimeError("invalid granule identity or Earthdata URL")
        keys.append((int(m[1]),m[2]))
    if len(set(keys)) != len(keys) or set(keys) != expected:
        raise RuntimeError(f"incomplete seven-spacecraft inventory: missing {sorted(expected-set(keys))}; unexpected {sorted(set(keys)-expected)}")
    return sorted(EXPECTED_SPACECRAFT)
...
def granule_rows(blob: bytes, title: str):
    ...
    if t_rel.ndim != 1 or t_rel.size == 0:
        raise RuntimeError(f"{title}: empty or invalid PVT time axis")
    ...
    valid = (pos != POS_FILL).all(axis=1) & (vel != VEL_FILL).all(axis=1) & np.isfinite(pos).all(axis=1) & np.isfinite(vel).all(axis=1) & np.isfinite(t_rel[keep])
    if not valid.any():
        raise RuntimeError(f"{title}: no valid navigation fixes")
    fm = int(fields["spacecraft_num"])
    match = re.match(r"cyg(\d{2})\.", title)
    if match and fm != int(match[1]):
        raise RuntimeError("spacecraft number disagrees with granule title")
...
def main() -> int:
    ...
    parser.add_argument("--plan-only", action="store_true", help="write the checked 28-granule access manifest without requesting protected data")
    parser.add_argument("--manifest", help="access/provenance JSON path (default: output filename plus .manifest.json)")
    ...
    validate_inventory(granules)
    manifest = {"collection": SHORT_NAME, "window": [WINDOW_START, WINDOW_END], "spacecraft": sorted(EXPECTED_SPACECRAFT), "variables": list(DAP4_VARIABLES), "granules": granules, "data_complete": False}
    manifest_path = Path(args.manifest or (args.out+".manifest.json"))
    atomic_bytes(manifest_path, json.dumps(manifest, indent=2).encode())
    if args.plan_only:
        print(f"Access plan verified: 28 granules, seven spacecraft. Wrote {manifest_path}")
        return 0
    ...
    for granule in sorted(granules, key=lambda g: (g["title"].split(".")[0], g["time_start"])):
        ...
        receipts.append({"title": granule["title"], "sha256": hashlib.sha256(blob).hexdigest(), "rows": len(rows["pvt_unix_seconds"]), "valid_rows": int(rows["valid_fix"].sum())})
    ...
    try:
        feather.write_feather(table, tmp, compression="lz4")
        os.replace(tmp,out)
    finally:
        tmp.unlink(missing_ok=True)
    manifest.update(data_complete=True, receipts=receipts, output_sha256=hashlib.sha256(out.read_bytes()).hexdigest(), rows=table.num_rows)
    atomic_bytes(manifest_path, json.dumps(manifest,indent=2).encode())
