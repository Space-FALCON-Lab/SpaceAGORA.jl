...
const DEFAULT_MANIFEST_PATH = joinpath(@__DIR__, "telemetry_benchmark_manifest.toml")
...
function parse_cli(args::Vector{String})::StudyConfig
    ...
    while i <= length(args)
        ...
        elseif startswith(arg, "--enforce=")
            ...
            throw(ArgumentError("Unsupported argument '$arg'. Supported: [quick|full], --profile=..., --manifest=..., --out-summary=..., --out-errors=..., --enforce=true|false"))
        end
        i += 1
    end
...
function _extract_extrema_series(df::DataFrame, planet, altitude_mode::Symbol)
    ...
    return (
        peri=(orbit=peri_orbit, altitude=peri_alt, time_s=peri_t),
        apo=(orbit=apo_orbit, altitude=apo_alt, time_s=apo_t)
    )
end
...
@inline _value_units(cfg::OrbitEventsScenarioConfig, event::String) = get(cfg.units_y, event, "km")
@inline _value_units(cfg::TimeAlignedScenarioConfig, event::String) = get(cfg.units_y, event, "km")
...
function _evaluate_thresholds(row, cfg::AbstractScenarioConfig, profile::Symbol)
    ...
    tol = get(tmap, row.event, nothing)
