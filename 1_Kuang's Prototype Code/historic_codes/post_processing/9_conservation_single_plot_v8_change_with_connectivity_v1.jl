...
p1 = plot(legend=false, grid=true, tick_direction=:out,
          ...
          title="Linear Momentum Change",
          titlefontsize=title_fontsize,
          top_margin=title_top_margin_mm*Plots.mm)
_draw_panel!(p1, x_axis, all_ΔP, mean_ΔP, std_ΔP; scale=1.0)
let _v0 = mean_ΔP[1]
    scatter!(p1, [orbit_common[1]], [_v0], markersize=5, color=:black, label="")
    annotate!(p1, orbit_common[1], _v0, Plots.text(@sprintf("%.3g", _v0), 7, :left, :bottom, :black))
    if orbit_common[end] >= 1.0
        _v1 = interp1(orbit_common, mean_ΔP, 1.0)
        scatter!(p1, [1.0], [_v1], markersize=5, color=:black, label="")
        annotate!(p1, 1.0, _v1, Plots.text(@sprintf("%.3g", _v1), 7, :left, :bottom, :black))
    end
end
_yl = ylims(p1)
...
let _v0 = mean_ΔH[1]
    scatter!(p2, [orbit_common[1]], [_v0], markersize=5, color=:black, label="")
    annotate!(p2, orbit_common[1], _v0, Plots.text(@sprintf("%.3g", _v0), 7, :left, :bottom, :black))
    if orbit_common[end] >= 1.0
        _v1 = interp1(orbit_common, mean_ΔH, 1.0)
        scatter!(p2, [1.0], [_v1], markersize=5, color=:black, label="")
        annotate!(p2, 1.0, _v1, Plots.text(@sprintf("%.3g", _v1), 7, :left, :bottom, :black))
    end
end
_yl2 = ylims(p2)
...
p3 = plot(legend=false, grid=true, tick_direction=:out,
          xlabel="Number of orbits (target satellite)", ylabel="ΔE  [J]",
          xguidefontsize=xlabel_fontsize, yguidefontsize=ylabel_fontsize,
          title="Orbital Energy Change ",
          titlefontsize=title_fontsize,
          top_margin=panel_top_margin_mm*Plots.mm)
_draw_panel!(p3, x_axis, all_ΔE, mean_ΔE, std_ΔE; scale=1.0)
let _v0 = mean_ΔE[1]
    scatter!(p3, [orbit_common[1]], [_v0], markersize=5, color=:black, label="")
    annotate!(p3, orbit_common[1], _v0, Plots.text(@sprintf("%.3g", _v0), 7, :left, :bottom, :black))
    if orbit_common[end] >= 1.0
        _v1 = interp1(orbit_common, mean_ΔE, 1.0)
        scatter!(p3, [1.0], [_v1], markersize=5, color=:black, label="")
        annotate!(p3, 1.0, _v1, Plots.text(@sprintf("%.3g", _v1), 7, :left, :bottom, :black))
    end
end
_yl3 = ylims(p3)
...
# ── Panel 4: In Range ───────────────────────────────────────────────────
p4 = plot(legend=false, grid=true, tick_direction=:out,
          xlabel="Number of orbits (target satellite)", ylabel="In Range",
          ylims=(0,1), yticks=0:1,
          xguidefontsize=xlabel_fontsize, yguidefontsize=ylabel_fontsize,
          title="Satellites In Range",
          titlefontsize=title_fontsize,
          top_margin=panel_top_margin_mm*Plots.mm)
_draw_panel!(p4, x_axis, all_in_range, mean_in_range, std_in_range; scale=1.0)
_yl4 = ylims(p4)
annotate!(p4, mean(xlims(p4)), _yl4[2] - subtitle_drop_frac*(_yl4[2]-_yl4[1]),
          Plots.text(_subtitle_str, subtitle_fontsize, :center, :top))

# ── Combine and save ──────────────────────────────────────────────────────────
fig = plot(p1, p2, p3, p4, layout=grid(4, 1, heights=panel_heights), size=fig_size)
