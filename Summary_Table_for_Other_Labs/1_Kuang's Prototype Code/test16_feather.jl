...
function test16_feather(; smoke=false)
    ...
    satellite_order = vcat(params[:helper_ids], params[:target_ids])
    ...
    @testset "test16-feather native schema and round-trip" begin
        ...
        ### 6.3. Target-last spacecraft mapping, masses, and orbital elements ###
        ...
        @test isapprox(getproperty(table, Symbol("sc$(params[:N])_periapsis_altitude"))[1], opts.target_altitude_km*1e3)
        @test isapprox(table.sc1_periapsis_altitude[1], opts.helper_altitude_km*1e3)
        ...
        @test table.laser_active_helper[1] == 1
        @test all(helper -> helper == 0 || helper in 1:helper_num, table.laser_active_helper)
        ...
        @test Arrow.getmetadata(table)["target_id"] == string(helper_num + 1)
