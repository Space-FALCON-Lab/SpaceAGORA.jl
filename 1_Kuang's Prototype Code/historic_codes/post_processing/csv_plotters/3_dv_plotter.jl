...
const DV_LABELS = Dict(
    :R => "Δv_R (m/s)",
    :T => "Δv_T (m/s)",
    :N => "Δv_N (m/s)",
)
const DV_ROW = Dict(:R => 1, :T => 2, :N => 3)
...
function plot_dv_vs_N_t(component::Symbol;   # component = :R, :T, or :N
                        rebuild_cache::Bool=false,
                        show_plot::Bool=false,
                        save_plot::Bool=true,
                        clear_output::Bool=true,
                        fn_prefix::AbstractString="delta_v")
    dv_sym = "dv" * string(component)        # e.g. "dvR"
    dv_lbl = DV_LABELS[component]
    row    = DV_ROW[component]

    ## 1. Initialization
    output_dir = joinpath(@__DIR__, "..", "output", "images_reconstructed_from_csv") * "/"
    println("Plotting $(dv_sym) results from CSV reconstruction...")
    outdir = joinpath(output_dir, fn_prefix)
    mkpath(outdir)
    if clear_output
    ...
    data_dir          = joinpath(@__DIR__, "..", "data")
    Ns_requested      = vcat(2, collect(6:5:161))
    scatter_cache_csv = joinpath(data_dir, "$(dv_sym)_vs_N_t_target_cache.csv")
    ...
    if isempty(N_vals)
        ...
        for N_req in Ns_requested
            ...
            sol_N, _, p = load_timeseries_csv(first(matches))  # need p for delta_v_RTN_time_series
            #print(sol_N)

            # for k in 1:min(3, length(sol_N.t))
            #     println("t=$(sol_N.t[k]):  $(sol_N.u[k])")
            # end
            # println("p = $p")
            t_dv, dv_hist = delta_v_RTN_time_series(sol_N, p)
            # #print
            # dv = dv_hist[end] # target = last satellite
            # println(dv[:, 1:3])
            target_idx = length(dv_hist)          # target = last satellite
            vals = dv_hist[target_idx][row, :]    # select R, T, or N row
            npts = min(length(t_dv), length(vals))

            append!(N_vals,  fill(Float64(N_req), npts))
            append!(t_vals,  Float64.(t_dv[1:npts]))
            append!(dv_vals, Float64.(vals[1:npts]))
        end

        open(scatter_cache_csv, "w") do io
            writedlm(io, ["N" "t_s" dv_sym], ',')
            writedlm(io, hcat(N_vals, t_vals, dv_vals), ',')
        end
        println("Saved scatter cache to $(scatter_cache_csv)")
    end
    ...
    ## 3. Plotting
    fit_gpr_surface_and_plot(
        N_vals, t_vals, dv_vals;
        outdir=outdir,
        fn_prefix=fn_prefix,
        fig_name="$(dv_sym)_vs_N_t_gpr_surface.png",
        surface_csv_name="$(dv_sym)_vs_N_t_gpr_surface.csv",
        oe_label=dv_lbl,
        oe_symbol=dv_sym,
        show_plot=show_plot,
        save_plot=save_plot
    )
    println("Done plotting $(dv_sym).")
end
...
# ---- choose which Δv component(s) to plot ----
for (idx, comp) in enumerate([:R, :T, :N])
    plot_dv_vs_N_t(comp; clear_output=(idx == 1))
end
